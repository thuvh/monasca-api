...
from monasca_api.api.core.events import envents_envelope as ev_envelope
...
class EventPublisher(object):
    ...
    def __init__(self):
        self.topic = CONF.kafka.openstack_events_topic
    ...
    def _publish(self, messages):
        ...
        while True:
            try:
                self._kafka_publisher.publish(self.topic, messages)
                LOG.debug('Sent %d messages to topic %s', num_of_msg, self.topic)
                break
