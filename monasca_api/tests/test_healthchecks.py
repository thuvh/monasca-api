...
class TestHealthChecks(testing.TestBase):
    ...
    def test_should_report_not_healthy_if_one_service_not_healthy(self,
                                                                  ...
                                                                  _):
        test_list = [
            {'kafka': {'health': False, 'message': 'Unavailable'},
             'alarms_db': {'health': True, 'message': 'OK'},
             'netrics_db': {'health': True, 'message': 'OK'}
             },
            {'kafka': {'health': True, 'message': 'OK'},
             'alarms_db': {'health': False, 'message': 'Connection Error'},
             'netrics_db': {'health': True, 'message': 'OK'}
             },
            {'kafka': {'health': True, 'message': 'OK'},
             'alarms_db': {'health': True, 'message': 'OK'},
             'netrics_db': {'health': False, 'message': 'Error'}
             },
        ]

        for service in test_list:
            kafka_check.health_check.return_value = base.CheckResult(service['kafka']['health'],
                                                                     service['kafka']['message'])
            alarms_db_check.health_check.return_value = base.CheckResult(service['alarms_db']['health'],
                                                                         service['alarms_db']['message'])
            metrics_db_check.health_check.return_value = base.CheckResult(service['netrics_db']['health'],
