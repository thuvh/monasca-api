...
class TestMeasurements(base.BaseApiTestCase):
    def setUp(self):
        ...
        self.api.add_route('/v2.0/metrics/measurements',
                           self.metrics_resource)
    ...
    def test_get_measurements(self):
        ...
        response = self.simulate_request('/v2.0/metrics/measurements',
                                         headers={'X-Roles':
                                                  CONF.security.default_authorized_roles[0],
                                                  'X-Tenant-Id': TENANT_ID},
                                         method='GET',
                                         query_string='name=cpu.system_perc&'
...
class TestStatistics(base.BaseApiTestCase):
    def setUp(self):
        ...
        self.api.add_route('/v2.0/metrics/statistics',
                           self.metrics_resource)

    def test_get_statistics(self):
        expected_statistics = \
            {u'elements': [{u'name': u'cpu.percent',
                            u'columns':
                                [u'timestamp',
                                 u'avg'],
                            u'id': '0',
        ...
        response = self.simulate_request('/v2.0/metrics/statistics',
                                         headers={'X-Roles':
                                                  CONF.security.default_authorized_roles[0],
                                                  'X-Tenant-Id': TENANT_ID},
                                         method='GET',
...
class TestMetricsNames(base.BaseApiTestCase):
    def setUp(self):
        ...
        self.api.add_route('/v2.0/metrics/names',
                           self.metrics_resource)
    ...
    def test_get_metrics_names(self):
        ...
        response = self.simulate_request('/v2.0/metrics/names',
                                         headers={'X-Roles':
                                                  CONF.security.default_authorized_roles[0],
                                                  'X-Tenant-Id': TENANT_ID},
                                         method='GET')
...
class TestDimensionNames(base.BaseApiTestCase):
    def setUp(self):
        ...
        self.api.add_route('/v2.0/metrics/dimensions/names',
                           self.metrics_resource)
    ...
    def test_get_dimension_names(self):
        ...
        response = self.simulate_request('/v2.0/metrics/dimensions/names',
                                         headers={'X-Roles':
                                                  CONF.security.default_authorized_roles[0],
                                                  'X-Tenant-Id': TENANT_ID},
                                         method='GET')
...
class TestDimensionValues(base.BaseApiTestCase):
    def setUp(self):
        ...
        self.api.add_route('/v2.0/metrics/dimensions/values',
                           self.metrics_resource)

    def test_get_dimension_values(self):
        expected_dimension_names = \
            {u'elements': [
                {u'dimension_value': u'devstack'}]}

        return_value = self.metrics_repo_mock.return_value
        return_value.list_dimension_values.return_value = expected_dimension_names['elements']
        response = self.simulate_request('/v2.0/metrics/dimensions/values',
                                         headers={'X-Roles':
                                                  CONF.security.default_authorized_roles[0],
                                                  'X-Tenant-Id': TENANT_ID},
                                         method='GET',
                                         query_string='dimension_name=hostname')
        self.assertEqual(falcon.HTTP_200, self.srmock.status)
        self.assertThat(response, base.RESTResponseEquals(expected_dimension_names))
