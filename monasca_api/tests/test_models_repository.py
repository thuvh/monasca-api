...
class TestModelsDB(base.BaseTestCase):
    ...
    def test_oracle(self):
        ...
        expected = ("SELECT metric_dimension.dimension_set_id, "
                    "LISTAGG(metric_dimension.name "
                    "|| '=' || metric_dimension.value, ',') WITHIN GROUP "
                    "(ORDER BY metric_dimension.name || '=' "
                    "|| metric_dimension.value) AS dimensions "
                    "FROM metric_dimension GROUP BY "
                    "metric_dimension.dimension_set_id")
        ...
        expected = ("SELECT metric_dimension.dimension_set_id, "
                    ...
                    "WITHIN GROUP (ORDER BY metric_dimension.name ASC) "
                    "AS dimensions "
                    "FROM metric_dimension GROUP BY "
                    "metric_dimension.dimension_set_id")
        self.assertEqual(expected.replace('\n', ''), query.replace('\n', ''))
        ...
        expected = ("SELECT metric_dimension.dimension_set_id "
                    "FROM metric_dimension ORDER BY CASE WHEN "
                    "metric_dimension.dimension_set_id='A' "
                    "THEN 0 WHEN metric_dimension.dimension_set_id='B' "
                    "THEN 1 WHEN "
                    "metric_dimension.dimension_set_id='C' "
    ...
    def test_postgres(self):
        ...
        query = str(self.group_concat_md.compile(dialect=dialect))
        expected = ("SELECT metric_dimension.dimension_set_id, "
                    "STRING_AGG(metric_dimension.name "
                    "|| '=' || metric_dimension.value, ',' ) AS dimensions "
                    "FROM metric_dimension "
                    "GROUP BY metric_dimension.dimension_set_id")
        self.assertEqual(expected.replace('\n', ''), query.replace('\n', ''))
        ...
        expected = ("SELECT metric_dimension.dimension_set_id, "
                    "STRING_AGG(metric_dimension.name || '=' "
                    "|| metric_dimension.value, ',' "
                    "ORDER BY metric_dimension.name ASC) "
                    "AS dimensions FROM metric_dimension "
                    "GROUP BY metric_dimension.dimension_set_id")
        self.assertEqual(expected.replace('\n', ''), query.replace('\n', ''))
        ...
        expected = ("SELECT metric_dimension.dimension_set_id "
                    "FROM metric_dimension ORDER BY CASE WHEN "
                    "metric_dimension.dimension_set_id='A' "
                    "THEN 0 WHEN metric_dimension.dimension_set_id='B' THEN 1 "
                    "WHEN metric_dimension.dimension_set_id='C' THEN 2 "
                    "ELSE 3 END ASC")
        self.assertEqual(expected.replace('\n', ''), query.replace('\n', ''))
    ...
    def test_sybase(self):
        ...
        expected = ("SELECT metric_dimension.dimension_set_id "
                    "FROM metric_dimension "
                    "ORDER BY CASE WHEN metric_dimension.dimension_set_id='A' "
                    "THEN 0 WHEN metric_dimension.dimension_set_id='B' THEN 1 "
                    "WHEN metric_dimension.dimension_set_id='C' "
                    "THEN 2 ELSE 3 END ASC")
        self.assertEqual(expected.replace('\n', ''), query.replace('\n', ''))
