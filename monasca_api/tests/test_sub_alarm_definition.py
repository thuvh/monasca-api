...
class TestSubAlarmDefinition(base.BaseTestCase):
    def test_init_from_row(self):
        ...
        self.assertEqual(sub_alarm_d.id, sub_alarm_d_dict['id'])
        self.assertEqual(sub_alarm_d.alarm_definition_id, sub_alarm_d_dict['alarm_definition_id'])
        self.assertEqual(sub_alarm_d.metric_name, sub_alarm_d_dict['metric_name'])
        self.assertEqual(sub_alarm_d.dimensions_str, sub_alarm_d_dict['dimensions'])
        self.assertEqual(sub_alarm_d.dimensions, dimension_dict)
        self.assertEqual(sub_alarm_d.function, sub_alarm_d_dict['function'])
        self.assertEqual(sub_alarm_d.operator, sub_alarm_d_dict['operator'])
        self.assertEqual(sub_alarm_d.period, sub_alarm_d_dict['period'])
        self.assertEqual(sub_alarm_d.periods, sub_alarm_d_dict['periods'])
        self.assertEqual(sub_alarm_d.threshold, sub_alarm_d_dict['threshold'])
        self.assertEqual(sub_alarm_d.deterministic, True)
    ...
    def test_init_from_sub_expr(self):
        ...
        self.assertEqual(sub_alarm_d.periods, sub_alarm_d_dict['periods'])
        self.assertEqual(sub_alarm_d.threshold, sub_alarm_d_dict['threshold'])
        self.assertEqual(sub_alarm_d.deterministic, False)

    def test_init_from_row_and_sub_expr(self):
        sub_alarm_d_dict = {'id': '111',
                            'alarm_definition_id': '123',
                            'function': 'AVG',
                            'metric_name': 'darth.vader',
        ...
        self.assertRaises(Exception, sub_alarm_definition.SubAlarmDefinition,
                          sub_alarm_d_dict, sub_expr_list)
