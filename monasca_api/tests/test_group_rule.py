...
class GroupRuleTestBase(TestBase):
    ...
    def test_group_rule_update_no_id(self):
        group_rule = {
            "name": "test_group_rule_update_no_id",
            "expression": "group by hostname, service",
            "group_wait": "30s",
            "repeat_interval": "2h",
            "description": "test group rule update with no id",
            'alarm_actions': [],
            'ok_actions': [],
            'undetermined_actions': []
        }
    ...
    def test_group_rule_patch_no_id(self):
        group_rule = {
            "name": "test_group_rule_patch_no_id",
            "expression": "group by hostname, service"
        }
        self.simulate_request(
            "/v2.0/group-rules/",
    ...
    def test_group_rule_delete_no_id(self):
        self.simulate_request(
            "/v2.0/group-rules/",
            headers={'X-Roles': 'admin', 'X-Tenant-Id': TENANT_ID},
            method="DELETE")
