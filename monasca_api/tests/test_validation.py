...
class TestNotificationValidation(unittest.TestCase):
    ...
    def test_validation_exception_for_webhook_no_scheme(self):
        notification = {"name": "MyWebhook", "type": "WEBHOOK", "address": "//somedomein.com"}
        with self.assertRaises(schemas_exceptions.ValidationException) as ve:
            schemas_notifications.validate(notification)
        ex = ve.exception
        self.assertEqual("Address //somedomein.com does not have URL scheme", ex.message)
    ...
    def test_validation_exception_for_webhook_invalid_scheme(self):
        notification = {"name": "MyWebhook", "type": "WEBHOOK", "address": "ftp://somedomein.com"}
        with self.assertRaises(schemas_exceptions.ValidationException) as ve:
            schemas_notifications.validate(notification)
        ex = ve.exception
        self.assertEqual("Address ftp://somedomein.com scheme is not in ['http', 'https']", ex.message)
