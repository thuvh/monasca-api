...
from monasca_api.v2.common.schemas import exclusions_schema
...
def validate_action_list(notification_ids):
...
alarm_grouping_manager_schema = {
    Required('name'): All(Any(str, unicode), Length(max=255)),
    Required('matchers'): Any([unicode], [str]),
    Marker('group_wait'): Any(str, unicode),
    Marker('repeat_interval'): Any(str, unicode),
    Marker('exclusions'): exclusions_schema.exclusions_schema,
    Marker('actions'): validate_action_list}


def validate(msg, require_all=False):
    try:
        request_body_schema = Schema(alarm_grouping_manager_schema,
