...
def validate_value_meta(value_meta):
    ...
    # entries
    assert len(value_meta) <= VALUE_META_MAX_NUMBER, "ValueMeta entries must be {} or less".format(VALUE_META_MAX_NUMBER)
    # total length
    assert len(value_meta_string) <= VALUE_META_MAX_LENGTH, "ValueMeta name value combinations must be {} characters or less".format(VALUE_META_MAX_LENGTH)
    for name in value_meta:
        # name
        assert isinstance(name, (str, unicode)), "ValueMeta name must be a string"
        assert len(name) <= VALUE_META_NAME_MAX_LENGTH, "ValueMeta name must be {} characters or less".format(VALUE_META_NAME_MAX_LENGTH)
