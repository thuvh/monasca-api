...
class AlarmGroupingManagers(alarm_grouping_managers_api_v2.
                            AlarmGroupingManagersV2API, alarming.Alarming):
    ...
    def on_get(self, req, res, alarm_grouping_manager_id=None):
        if alarm_grouping_manager_id is None:
            ...
            result = self._alarm_grouping_manager_list(
    ...
    @resource.resource_try_catch_block
    def on_put(self, req, res, alarm_grouping_manager_id):
        ...
        result = self._alarm_grouping_manager_update_or_patch(
            req.project_id, alarm_grouping_manager_id, name, matchers,
    ...
    @resource.resource_try_catch_block
    def on_patch(self, req, res, alarm_grouping_manager_id):
        ...
        name = get_query_alarm_grouping_manager_name(alarm_grouping_manager,
                                                     return_none=True)
        matchers = get_query_alarm_grouping_manager_matchers(
            alarm_grouping_manager, return_none=True)
        group_wait = get_query_alarm_grouping_manager_group_wait(
            alarm_grouping_manager, return_none=True)
        repeat_interval = get_query_alarm_grouping_manager_repeat_interval(
            alarm_grouping_manager, return_none=True)
        ...
        result = self._alarm_grouping_manager_update_or_patch(
            req.project_id, alarm_grouping_manager_id, name,
            matchers, group_wait, repeat_interval, exclusions, actions,
            patch=True)
    ...
    @resource.resource_try_catch_block
    def on_delete(self, req, res, alarm_grouping_manager_id):

        helpers.validate_authorization(req, self._default_authorized_roles)
        self._alarm_grouping_manager_delete(req.project_id,
                                            alarm_grouping_manager_id)
        res.status = falcon.HTTP_204

    def _alarm_grouping_manager_delete(self, tenant_id,
                                       alarm_grouping_manager_id):
        if not self._alarm_grouping_managers_repo\
                .delete_alarm_grouping_manager(tenant_id,
                                               alarm_grouping_manager_id):
            raise falcon.HTTPNotFound

        self._send_alarm_grouping_manager_deleted_event(
            alarm_grouping_manager_id)

    def _send_alarm_grouping_manager_deleted_event(self,
                                                   alarm_grouping_manager_id):
        alarm_grouping_manager_deleted_event_msg = {
            u"alarm-grouping-manager-deleted": {u"id":
                                                alarm_grouping_manager_id}}
        self.send_event(self.events_message_queue,
                        alarm_grouping_manager_deleted_event_msg)

    def _alarm_grouping_manager_update_or_patch(
            self, tenant_id, alarm_grouping_manager_id, name, matchers,
            group_wait, repeat_interval, exclusions, actions, patch):
        if name:
            self._validate_name_not_conflicting(
                tenant_id, name, expected_id=alarm_grouping_manager_id)

        alarm_grouping_manager_row = (
            self._alarm_grouping_managers_repo.
            update_or_patch_alarm_grouping_manager(
                tenant_id,
                alarm_grouping_manager_id,
                name,
                matchers,
        ...
        result = self._build_alarm_grouping_manager_show_result(
            alarm_grouping_manager_row)
        # Not all of the passed in parameters will be set if this called
        # from on_patch vs on_update. The alarm-grouping-manager-updated event
        # MUST have all of the fields set so use the dict built from the
        # data returned from the database
        alarm_grouping_manager_event_dict = (
            {u'tenantId': tenant_id,
             ...
             u'actions': result['actions']})

        alarm_grouping_manager_updated_event = (
            {u'alarm-grouping-manager-updated': alarm_grouping_manager_event_dict})
    ...
    def _alarm_grouping_manager_create(self, tenant_id, name, matchers,
        ...
        self._send_alarm_grouping_manager_created_event(
            tenant_id, alarm_grouping_manager_id, name, matchers,
            group_wait, repeat_interval, exclusions, actions)
    ...
    def _build_alarm_grouping_manager_show_result(
        ...
        matchers = get_comma_separated_str_as_list(alarm_grouping_manager_row['matchers'])

        actions_list = get_comma_separated_str_as_list(alarm_grouping_manager_row['actions'])
        ...
        repeat_interval = (alarm_grouping_manager_row['repeat_interval']
                           if alarm_grouping_manager_row['repeat_interval'] is not None
                           else '2h')
    ...
    def _alarm_grouping_manager_list(self, tenant_id, name, req_uri, offset,
                                     limit):
        ...
        for alarm_grouping_manager_row in alarm_grouping_manager_rows:
            ...
            agm = {u'id': alarm_grouping_manager_row['id'],
                   ...
                   u'matchers': matchers,
                   u'group_wait': alarm_grouping_manager_row['group_wait']
                   if (alarm_grouping_manager_row['group_wait']) else u'30s',
                   u'repeat_interval': alarm_grouping_manager_row['repeat_interval']
                   if (alarm_grouping_manager_row['repeat_interval']) else u'2h',
                   u'actions': actions_list,
                   u'exclusions': exclusions}
    ...
    def _send_alarm_grouping_manager_created_event(
            self, tenant_id, alarm_grouping_manager_id, name, matchers,
            group_wait, repeat_interval, exclusions, actions):
