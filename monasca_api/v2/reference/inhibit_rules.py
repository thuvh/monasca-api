...
class InhibitRules(inhibit_rules_api_v2.InhibitRulesV2API, alarming.Alarming):
    ...
    def on_post(self, req, res):
        ...
        result = self._inhibit_rule_create(req.project_id, name, description,
                                           expression)
    ...
    def on_patch(self, req, res, inhibit_rule_id):
        ...
        result = self._inhibit_rule_update_or_patch(
            req.project_id, inhibit_rule_id, name, description,
            expression, patch=True)
    ...
    def _inhibit_rule_update_or_patch(self, tenant_id, inhibit_rule_id, name,
        ...
        inhibit_rule_event_dict = (
            {u'tenantId': tenant_id,
             ...
             u'expression': result['expression']})
    ...
    def _validate_inhibit_rule(self, inhibit_rule, require_all=False):
        try:
            ...
            for key in inhibit_rule['expression']:
    ...
    @resource.resource_try_catch_block
    def _inhibit_rule_create(self, tenant_id, name, description, expression):
        self._validate_name_not_conflicting(tenant_id, name)
        inhibit_rule_id = (
            self._inhibit_rules_repo
                .create_inhibit_rule(tenant_id, name, description, expression))

        self._send_inhibit_rule_created_event(
            tenant_id, inhibit_rule_id, name, expression)
    ...
    def _build_inhibit_rule_show_result(
            self, inhibit_rule_row):

        expression = get_comma_separated_str_as_list(
            inhibit_rule_row['expression'])
    ...
    def _inhibit_rule_list(self, tenant_id, name, req_uri, offset, limit):
        ...
        for row in inhibit_rule_rows:
            expression = get_comma_separated_str_as_list(row['expression'])
    ...
    def _send_inhibit_rule_created_event(
            self, tenant_id, inhibit_rule_id, name, expression):
