...
class InhibitRules(inhibit_rules_api_v2.InhibitRulesV2API, alarming.Alarming):
    ...
    def on_get(self, req, res, inhibit_rule_id=None):
        if inhibit_rule_id is None:
            ...
            if offset is not None and not isinstance(offset, six.integer_types):
                try:
                    offset = int(offset)
                except Exception:
    ...
    @resource.resource_try_catch_block
    def on_put(self, req, res, inhibit_rule_id=None):
        if not inhibit_rule_id:
            raise HTTPBadRequestError(
                'Bad Request', 'Inhibit rule id is not provided')
        helpers.validate_authorization(req, self._default_authorized_roles)
    ...
    @resource.resource_try_catch_block
    def on_patch(self, req, res, inhibit_rule_id=None):
        if not inhibit_rule_id:
            raise HTTPBadRequestError(
                'Bad Request', 'Inhibit rule id is not provided')
        helpers.validate_authorization(req, self._default_authorized_roles)
    ...
    @resource.resource_try_catch_block
    def on_delete(self, req, res, inhibit_rule_id=None):
        if not inhibit_rule_id:
            raise HTTPBadRequestError(
                'Bad Request', 'Inhibit rule id is not provided')
        helpers.validate_authorization(req, self._default_authorized_roles)
...
def get_query_inhibit_rule_description(inhibit_rule, return_none=False):
    if 'description' in inhibit_rule:
        ...
        return constants.INHIBIT_RULE_DESCRIPTION
