...
METRIC_COUNT = Counter('monasca_api_metric_insert_count',
                       ...
                       ['_aggregate'])
METRIC_TIMESTAMP_REJECTED = Counter('monasca_api_rejected_metric_timestamp_count',
                          'Metrics rejected by API becaue of timestamp',
                          ['_aggregate'])
        ...
        with API_TIME.labels('post', 'metrics', 'sum').time(), \
            ...
            try:
                ...
                if 'out of legal range' in ex.message:
                    METRIC_TIMESTAMP_REJECTED.inc(len(metrics))
