    ...
    def _validate_single_metric(self, metric):
        ...
        if "dimensions" in metric:
            for dimension_key in metric['dimensions']:
                dimension_value = metric['dimensions'][dimension_key]
                validation.dimension_key(dimension_key)
                if type(dimension_value) is not str:
                    dimension_value = str(dimension_value)
                    LOG.warn("Converting dimension value to string '{}'".format(dimension_value))
                validation.dimension_value(dimension_value)
