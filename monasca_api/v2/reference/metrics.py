...
VALIDATE_TIME = Summary('monasca_api_batch_validate_time', 'Batch validation time')
VALIDATE_LATENCY = MeanGauge('monasca_api_validate_latency_mean',
                             'Mean time spent validating batches',
                             multiprocess_mode='liveall',
                             interval=datetime.timedelta(minutes=1))

BATCH_TIME = Summary('monasca_api_batch_time', 'Kafka batch insert time')
BATCH_LATENCY = MeanGauge('monasca_api_batch_latency_mean',
                          'Mean time spent inserting into kafka per batch',
                          multiprocess_mode='liveall',
                          interval=datetime.timedelta(minutes=1))

MESSAGE_TIME = Summary('monasca_api_message_time', 'Kafka batch insert time per message')
...
class Metrics(metrics_api_v2.MetricsV2API):
    ...
    def _send_metrics(self, metrics):
        ...
        BATCH_LATENCY.sample(elapsed)
        MESSAGE_TIME.observe(elapsed / len(metrics))
        MESSAGE_LATENCY.sample(elapsed / len(metrics))
    ...
    def on_post(self, req, res):
        with API_TIME.labels('post', 'metrics').time(), \
             API_LATENCY.labels('post', 'metrics').time():
    ...
    def on_get(self, req, res):
        with API_TIME.labels('get', 'metrics').time(), \
             API_LATENCY.labels('get', 'metrics').time():
...
class MetricsMeasurements(metrics_api_v2.MetricsMeasurementsV2API):
    ...
    def on_get(self, req, res):
        with API_TIME.labels('get', 'measurements').time(), \
             API_LATENCY.labels('get', 'measurements').time():
...
class MetricsStatistics(metrics_api_v2.MetricsStatisticsV2API):
    ...
    def on_get(self, req, res):
        with API_TIME.labels('get', 'metrics_statistics').time(), \
             API_LATENCY.labels('get', 'metrics_statistics').time():
...
class MetricsNames(metrics_api_v2.MetricsNamesV2API):
    ...
    def on_get(self, req, res):
        with API_TIME.labels('get', 'metrics_names').time(), \
             API_LATENCY.labels('get', 'metrics_names').time():
...
class DimensionValues(metrics_api_v2.DimensionValuesV2API):
    ...
    def on_get(self, req, res):
        with API_TIME.labels('get', 'dimension_values').time(), \
             API_LATENCY.labels('get', 'dimension_values').time():
...
class DimensionNames(metrics_api_v2.DimensionNamesV2API):
    ...
    def on_get(self, req, res):
        with API_TIME.labels('get', 'dimension_names').time(), \
             API_LATENCY.labels('get', 'dimension_names').time():
