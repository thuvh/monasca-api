...
class AlarmInhibitionDefinitions(alarm_inhibition_definitions_api_v2.
    ...
    def on_post(self, req, res):
        ...
        alarm_inhibition_definition = helpers.from_json(req)
        ...
        description = get_query_alarm_inhibition_definition_description(
    ...
    def on_put(self, req, res, alarm_inhibition_definition_id):
        ...
        result = self._alarm_inhibition_definition_update_or_patch(
            req.project_id, alarm_inhibition_definition_id, name, description,
            equal, source_match, target_match, exclusions, patch=False)
    ...
    def on_patch(self, req, res, alarm_inhibition_definition_id):
        ...
        result = self._alarm_inhibition_definition_update_or_patch(
            req.project_id, alarm_inhibition_definition_id, name, description,
            equal, source_match, target_match, exclusions, patch=True)
    ...
    def _alarm_inhibition_definition_update_or_patch(
            self, tenant_id, alarm_inhibition_definition_id, name,
            description, equal, source_match, target_match, exclusions, patch):
        if name:
    ...
    @resource.resource_try_catch_block
    def _alarm_inhibition_definition_create(self, tenant_id, name, description,
                                            equal, source_match, target_match,
                                            exclusions):
        self._validate_name_not_conflicting(tenant_id, name)
        alarm_inhibition_definition_id = (
            self._alarm_inhibition_definitions_repo
                .create_alarm_inhibition_definition(tenant_id, name, description,
                                                    equal, source_match,
                                                    target_match, exclusions))
    ...
    def _build_alarm_inhibition_definition_show_result(
        ...
        source_match = get_comma_separated_str_as_dict(
            alarm_inhibition_definition_row['source_match'])
        target_match = get_comma_separated_str_as_dict(
            alarm_inhibition_definition_row['target_match'])
        exclusions = get_comma_separated_str_as_dict(
            alarm_inhibition_definition_row['exclusion'])

        result = {
            u'id': alarm_inhibition_definition_row['id'],
            u'name': alarm_inhibition_definition_row['name'],
            u'description': alarm_inhibition_definition_row['description'],
            u'equal': equal,
            u'source_match': source_match,
            u'target_match': target_match,
            u'exclusions': exclusions}
    ...
    def _alarm_inhibition_definition_list(self, tenant_id, name, req_uri,
        ...
        for row in alarm_inhibition_definition_rows:
            ...
            source_match = get_comma_separated_str_as_dict(row['source_match'])
            target_match = get_comma_separated_str_as_dict(row['target_match'])
            exclusions = get_comma_separated_str_as_dict(row['exclusion'])
...
def get_query_alarm_inhibition_definition_description(alarm_inhibition_definition,
                                                      return_none=False):
    if 'description' in alarm_inhibition_definition:
        return alarm_inhibition_definition['description']
    else:
        if return_none:
            return None
        else:
...
def get_comma_separated_str_as_dict(comma_separated_str):
    if not comma_separated_str:
        ...
        return dict(part.split("=")
                    for part in comma_separated_str.decode('utf8').split(","))
