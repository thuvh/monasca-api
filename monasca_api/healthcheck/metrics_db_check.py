...
class MetricsDbCheck(base.BaseHealthCheck):
    ...
    Health check what type of database is used (InfluxDB, Cassandra)
    and provide health according to the given db.

    Health for InfluxDB verifies if:
    * check the db status by the /ping endpoint.

    Health for the Cassandra verifies if:
    ...
    * keyspace exists
    ...
    def health_check(self):
        ...
        return base.CheckResult(health=status[0],
                                message=status[1])
