...
def get_wsgi_app():
    ...
    return deploy.loadapp(
        'config:%s/api-paste.ini' % config_dir,
        relative_to='./',
        name='main',
        global_conf={
            'config_file': '%s/api-config.conf' % config_dir,
            'log_config_file': '%s/api-logging.conf' % config_dir
        }
    )
...
def main():
    ...
    server_version = 'api/%s' % version.version_str
