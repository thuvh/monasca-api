...
from monasca_api.common.repositories.sql import sql_repository
from sqlalchemy import MetaData, update, delete, select, text, bindparam, func, literal_column
...
class AlarmsRepository(sql_repository.SQLRepository,
    ...
    def __init__(self):
        ...
        self.base_query_from = (a_s.join(ad, ad.c.id == a_s.c.alarm_definition_id)
                                ...
                                .join(mdg, mdg.c.dimension_set_id == mdd.c.metric_dimension_set_id))
        ...
        self.get_a_query = (select([a_s.c.state])
    ...
    @sql_repository.sql_try_catch_block
    def get_alarms(self, tenant_id, query_parms={}, offset=None, limit=None):
        ...
        with self._db_engine.connect() as conn:
            ...
            if 'metric_dimensions' in query_parms:
                ...
                for i, metric_dimension in enumerate(query_parms['metric_dimensions']):
                    ...
                    if '|' in parsed_dimension[1] and len(parsed_dimension) > 1:
            ...
            if 'sort_by' in query_parms:
                order_columns = [literal_column(col) for col in query_parms['sort_by']]
    ...
    def get_alarms_count(self, tenant_id, query_parms, offset, limit):
        ...
        with self._db_engine.connect() as conn:
            ...
            query_from = a_s.join(ad, ad.c.id == a_s.c.alarm_definition_id)
            ...
            if 'group_by' in query_parms:
                ...
                if metric_group_by:
                    sub_query_columns = [a.c.id]
                    if 'metric_name' in metric_group_by:
                        group_by_columns.append(md.c.name.label('metric_name'))
                    if 'dimension_name' in metric_group_by:
                        group_by_columns.append(mdim.c.name.label('dimension_name'))
                    if 'dimension_value' in metric_group_by:
                        group_by_columns.append(mdim.c.value.label('dimension_value'))

                    sub_query_columns.extend(group_by_columns)

                    sub_query_from = (md.join(mdd, md.c.id == mdd.c.metric_definition_id)
                                      .join(mdim, mdd.c.metric_dimension_set_id == mdim.c.dimension_set_id)
