...
def _group_concat_oracle(element, compiler_, **kw):
    ...
    if element.order_by is not None and len(element.order_by) > 0:
        str_order_by = "ORDER BY {0}".format(",".join([str(x) for x in element.order_by]))
    else:
        str_order_by = "ORDER BY {0}".format(",".join([str(x) for x in element.columns]))
    return "LISTAGG({0}, '{2}') WITHIN GROUP ({1})".format(
        ", ".join([str(x) for x in element.columns]),
        str_order_by,
        element.separator,
    )
...
def _group_concat_sqlite(element, compiler_, **kw):
