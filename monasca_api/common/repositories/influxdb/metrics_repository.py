...
class MetricsRepository(metrics_repository.AbstractMetricsRepository):
    ...
    def _generate_dimension_names_id(self, metric_name):
        sha1 = hashlib.sha1()
        hashstr = "metricName=" + (metric_name or "")
        sha1.update(hashstr)
        return sha1.hexdigest()
    ...
    def _build_serie_dimension_names(self, series_names, metric_name,
                                     tenant_id, region, offset):
        dim_names = []
        sha1_id = self._generate_dimension_names_id(metric_name)
        json_dim_names = {u'id': sha1_id,
                          u'dimension_names': dim_names}
    ...
    def list_dimension_names(self, tenant_id, region, metric_name, offset,
                             limit):
        try:
            query = self._build_show_series_query(None, metric_name, tenant_id, region)
            result = self.influxdb_client.query(query)
