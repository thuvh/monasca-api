...
class MetricsRepository(metrics_repository.AbstractMetricsRepository):
    ...
    def _build_select_all_query(self, dimensions, name, tenant_id, region, start_timestamp, end_timestamp, no_record_check_dim):

        from_clause = self._build_from_clause(dimensions, name, tenant_id,
                                              region, start_timestamp,
                                              end_timestamp)
        if no_record_check_dim != None:
    ...
    def list_dimension_values(self, tenant_id, region, metric_name,
        ...
        try:
            ...
            json_dim_value_list_filtered = \
                sorted(json_dim_value_list_filtered,
                    key=lambda dim_value_dict: \
                    dim_value_dict['dimension_value'])
    ...
    def list_dimension_names(self, tenant_id, region, metric_name,
        ...
        try:
            ...
            if metric_name != None:
                ...
                json_dim_name_list_filtered = \
                    sorted(json_dim_name_list_filtered,
                        key=lambda dim_name_dict: \
                        dim_name_dict['dimension_name'])
