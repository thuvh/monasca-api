...
class MetricsRepository(metrics_repository.AbstractMetricsRepository):
    ...
    def _init_serie_builders(self):
        ...
        try:
            ...
            if influxdb_version < version.StrictVersion('0.11.0'):
                self._init_serie_builders_v0_9_5()
            else:
                self._init_serie_builders_v0_11_0()
        except Exception as ex:
            LOG.exception(ex)
            # Initialize the serie builders for v0_9_5. Not sure when SHOW DIAGNOSTICS added
            # support for a version string so to address backward compatibility initialize
            # InfluxDB serie builders <  v0.11.0
            self._init_serie_builders_v0_9_5()

    def _init_serie_builders_v0_9_5(self):
    ...
    def _init_serie_builders_v0_11_0(self):
        '''Initialize function for InfluxDB serie builders >= v0.11.0.
    ...
    def _get_influxdb_version(self):
        '''If found valid Version in the result, return the InfluxDB Version,
        otherwise raise an exception. InfluxDB has changed the format of their
        result set and SHOW DIAGNOSTICS was introduced at some point, so 0.0.0
        should hopefully be sufficient for maintaining compatibility with prior
        releases.
        '''
        try:
