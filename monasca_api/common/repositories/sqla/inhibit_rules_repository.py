...
class InhibitRulesRepository(sql_repository.SQLRepository,
    ...
    def __init__(self):
        ...
        self.base_query = (select([ir_s.c.rule_id,
                                   ir_s.c.name,
                                   ir_s.c.expression,
                                   ir_s.c.description]))

        self.insert_ir_query = \
            (insert(ir).values(rule_id=bindparam('b_rule_id'),
        ...
        self.update_ir_query = (
            update(ir)
            .where(ir.c.tenant_id == bindparam('b_tenant_id'))
            .where(ir.c.rule_id == bindparam('b_rule_id')))
    ...
    @sql_repository.sql_try_catch_block
    def get_inhibit_rules(self, tenant_id, name=None, offset=None, limit=1000):
    ...
    @sql_repository.sql_try_catch_block
    def get_inhibit_rule(self, tenant_id, _id):
        with self._db_engine.connect() as conn:
            return self._get_inhibit_rule(conn, tenant_id, _id)

    @sql_repository.sql_try_catch_block
    def update_or_patch_inhibit_rule(
            self, tenant_id, inhibit_rule_id, name, expression, description,
            patch=False):
    ...
    def _get_inhibit_rule(self, conn, tenant_id, _id):
        ...
        query = (self.base_query
                 .where(irs.c.tenant_id == bindparam('b_tenant_id'))
                 .where(irs.c.rule_id == bindparam('b_rule_id'))
                 .where(irs.c.deleted_at == null()))

        row = conn.execute(query,
                           b_tenant_id=tenant_id,
                           b_rule_id=_id).fetchone()
