...
class AlarmInhibitionDefinitionsRepository(sql_repository.SQLRepository, aidr.AlarmInhibitionDefinitionsRepository):
    def __init__(self):
        ...
        aidsmg = (
            select([aid_sm.c.alarm_inhibition_definition_id, models.group_concat(
                aidsm_columns).label('source_match')]).select_from(aid_sm).
            group_by(aid_sm.c.alarm_inhibition_definition_id).alias('aidsmg'))
        ...
        self.insert_aid_query = \
            (insert(aid).values(id=bindparam('b_id'),
        ...
        self.update_aid_query = (
            update(aid).where(aid.c.tenant_id == bindparam('b_tenant_id'))
                       .where(aid.c.id == bindparam('b_id')))

        self.delete_aidsm_query = (
            delete(aid_sm)
            .where(aid_sm.c.alarm_inhibition_definition_id == bindparam(
                'b_alarm_inhibition_definition_id')))

        self.delete_aidtm_query = (
            delete(aid_tm)
            .where(aid_tm.c.alarm_inhibition_definition_id == bindparam(
                'b_alarm_inhibition_definition_id')))

        self.delete_aide_query = (
            delete(aid_e)
            .where(aid_e.c.alarm_inhibition_definition_id == bindparam(
                'b_alarm_inhibition_definition_id')))

        self.soft_delete_aid_query = (update(aid).where(
            aid.c.tenant_id == bindparam('b_tenant_id'))
            .where(aid.c.id == bindparam('b_id'))
            .where(aid.c.deleted_at == null())
            .values(deleted_at=datetime.datetime.utcnow()))

        self.delete_ar_query = (
            delete(ar)
            .where(ar.c.alarm_rule_id == bindparam(
                'b_alarm_inhibition_definition_id')))

    @sql_repository.sql_try_catch_block
    def create_alarm_inhibition_definition(self, tenant_id, name, equal,
    ...
    def get_alarm_inhibition_definitions(self, tenant_id, name=None,
        ...
        with self._db_engine.connect() as conn:
            ...
            query = (self.base_query
                     ...
                     .where(aid.c.tenant_id == bindparam('b_tenant_id'))
                     .where(aid.c.deleted_at == null()))
            if name:
                query = query.where(aid.c.name == bindparam('b_name'))
    ...
    def update_or_patch_alarm_inhibition_definition(
        ...
        with self._db_engine.begin() as conn:
            ...
            if patch:
                if source_match:
                    self._delete_alarm_inhibition_definition_source_match(
                        conn, aid_id)
            else:
                conn.execute(self.delete_aidsm_query,
            ...
            if patch:
                if target_match:
                    self._delete_alarm_inhibition_definition_target_match(
                        conn, aid_id)
            else:
                conn.execute(
                    self.delete_aidtm_query,
            ...
            self._insert_into_target_match(
                conn, aid_id, target_match)

            # Delete old exclusions
            if patch:
                if exclusions:
                    self.delete_alarm_inhibition_definition_exclusions(
                        conn, aid_id)
                else:
                    conn.execute(
                        self.delete_aide_query,
            ...
            # Delete old alarm rule id relationships
            conn.execute(
                self.delete_ar_query,
                b_alarm_inhibition_definition_id=aid_id)
    ...
    def delete_alarm_inhibition_definition(self, tenant_id,
        ...
        with self._db_engine.begin() as conn:
            cursor = conn.execute(self.soft_delete_aid_query,
                                  b_tenant_id=tenant_id,
                                  b_id=aid_id)
    ...
    def _delete_alarm_inhibition_definition_source_match(self, conn, _id):
        conn.execute(self.delete_aidsm_query,
                     b_alarm_inhibition_definition_id=_id)

    def _delete_alarm_inhibition_definition_target_match(self, conn, _id):
        conn.execute(self.delete_aidtm_query,
                     b_alarm_inhibition_definition_id=_id)

    def _delete_alarm_inhibition_definition_exclusions(self, conn, _id):
        conn.execute(self.delete_aid_exclusion_query,
                     b_alarm_inhibition_definition_id=_id)
