...
class AlarmInhibitionDefinitionsRepository(sql_repository.SQLRepository, aidr.AlarmInhibitionDefinitionsRepository):
    ...
    def update_or_patch_alarm_inhibition_definition(
        ...
        with self._db_engine.begin() as conn:
            ...
            if new_equal != original_row['equal']:
                msg = "equal must not change {} vs {}.".format(equal, original_row['equal']).encode('utf8')
                raise exceptions.InvalidUpdateException(msg)

            if not source_match:
                new_source_match = original_row['source_match']
            else:
                new_source_match = ",".join(['='.join([key, str(value)])
                                            for key, value in source_match.items()])
            if new_source_match != original_row['source_match']:
                msg = "source match must not change.".encode('utf8')
                raise exceptions.InvalidUpdateException(msg)

            if not target_match:
                new_target_match = original_row['target_match']
            else:
                new_target_match = ",".join(['='.join([key, str(value)])
                                            for key, value in target_match.items()])
            if new_target_match != original_row['target_match']:
                msg = "target match must not change.".encode('utf8')
                raise exceptions.InvalidUpdateException(msg)

            if not exclusions:
                new_exclusions = original_row['exclusion']
            else:
                new_exclusions = ",".join(['='.join([key, str(value)])
                                          for key, value in exclusions.items()])
            if new_exclusions != original_row['exclusion']:
