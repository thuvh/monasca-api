...
class AlarmInhibitionManagersRepository(sql_repository.SQLRepository,
    ...
    def __init__(self):
        ...
        aimsmg = (
            select([aim_sm.c.alarm_inhibition_manager_id, models.group_concat(
                aimsm_columns).label('source_match')]).select_from(aim_sm).
            group_by(aim_sm.c.alarm_inhibition_manager_id).alias('aimsmg'))
