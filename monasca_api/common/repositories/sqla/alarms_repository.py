        ...
        with self._db_engine.connect() as conn:
            ...
            if 'lifecycle_state' in query_parms:
                ...
                parms['b_lifecycle_state'] = query_parms['lifecycle_state'].encode('utf8') \
                    if six.PY2 else query_parms['lifecycle_state']

            if 'link' in query_parms:
                query = query.where(a.c.link == bindparam('b_link'))
                parms['b_link'] = query_parms['link'].encode('utf8') if six.PY2 \
                    else query_parms['link']
            ...
            if 'state_updated_start_time' in query_parms:
                ...
                date_str = query_parms['state_updated_start_time'].encode('utf8') if six.PY2 \
                    else query_parms['state_updated_start_time']
