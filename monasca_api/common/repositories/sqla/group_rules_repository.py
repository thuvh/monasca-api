...
class GroupRulesRepository(sql_repository.SQLRepository,
    ...
    def __init__(self):
        ...
        gr_action_a = (select([gr_aa.c.group_rule_id,
                       models.group_concat([gr_aa.c.action_id]).label(
                           'alarm_actions')])
                       .select_from(gr_aa)
        ...
        gr_action_o = (select([gr_ao.c.group_rule_id, models.group_concat([gr_ao.c.action_id]).
                               label('ok_actions')])
                       .select_from(gr_ao)
        ...
        self.base_query_from = (
            gr_s.outerjoin(gr_action_a,
                           gr_action_a.c.group_rule_id == gr_s.c.rule_id)
                .outerjoin(gr_action_o,
                           gr_action_o.c.group_rule_id == gr_s.c.rule_id)
                .outerjoin(gr_action_u,
                           gr_action_u.c.group_rule_id == gr_s.c.rule_id))

        self.base_query = (select([gr_s.c.rule_id,
                                   gr_s.c.name,
                                   gr_s.c.expression,
                                   gr_s.c.group_wait,
        ...
        self.insert_gr_query = \
            (insert(gr).values(rule_id=bindparam('b_rule_id'),
        ...
        self.update_gr_query = (
            ...
            .where(gr.c.rule_id == bindparam('b_rule_id')))

        self.delete_gr_query = (
            delete(gr_s)
            .where(gr_s.c.rule_id == bindparam('b_rule_id')))

        self.delete_gr_action_state_query = (
            delete(gr_a)
            .where(gr_a.c.group_rule_id ==
                   bindparam('b_group_rule_id'))
    ...
    @sql_repository.sql_try_catch_block
    def get_group_rules(self, tenant_id, name=None, offset=None, limit=1000):
    ...
    @sql_repository.sql_try_catch_block
    def get_group_rule(self, tenant_id, _id):
        with self._db_engine.connect() as conn:
            return self._get_group_rule(conn, tenant_id, _id)
    ...
    def update_or_patch_group_rule(
        ...
        with self._db_engine.begin() as conn:
            ...
            if not updated_row:
                raise Exception("Failed to find current group rule")
    ...
    def _get_group_rule(self, conn, tenant_id, _rule_id):
        gr = self.gr
        query = (self.base_query
                 .select_from(self.base_query_from)
                 .where(gr.c.tenant_id == bindparam('b_tenant_id'))
    ...
    def _insert_into_actions(self, conn, group_rule_id, actions,
        ...
        for action in actions:
            row = conn.execute(self.select_nm_query,
                               b_rule_id=action.encode('utf8')).fetchone()
            if not row:
