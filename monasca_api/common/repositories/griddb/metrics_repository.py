...
Metric = collections.namedtuple(
    "Metric",
    ["metric_hash", "metric_map"]
)

Measurement = collections.namedtuple(
    "Measurement",
    ["timestamp", "metric_hash", "metric_map", "value", "value_meta"]
)

AlarmStateHistory = collections.namedtuple(
    "AlarmStateHistory",
...
class MetricsRepository(metrics_repository.AbstractMetricsRepository):
    ...
    def _get_time_series(self, *args):
        """Get time series object of GridDB.
    ...
    def _get_metrics(self, tenant_id, region,
        ...
        return [Metric(_hash, _map) for _hash, _map in sorted(metrics.items())]
    ...
    def _get_alarm_histories(self, tenant_id,
        ...
        while rs.has_next():
            rs.get_next(row)
            alarm_state_histories.append(AlarmStateHistory(
                row.get_field_as_timestamp(0),  # timestamp
            ...
            for key, value in dimensions.items():
                ...
                for metric_hash, metric_map in metrics:
                    ...
                    if values is None or metric_map[key] in values:
                        new_metrics.append(Metric(metric_hash, metric_map))
                metrics = new_metrics

        # Filter metrics with offset
        metric_hashes = [_.metric_hash for _ in metrics]
        ...
        if dimensions:
            for d_key, d_value in dimensions.items():
                ...
                for m in measurements:
                    if d_key not in m.metric_map:
                        break
                    if d_values is None or m.metric_map[d_key] in d_values:
    ...
    def alarm_history(self, tenant_id, alarm_id_list,
        ...
        if offset is not None:
            offset = float(offset)
            ids = [_.id for _ in alarm_histories]
