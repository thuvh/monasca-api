...
def hash_int_to_str(hash_long):
    """Convert an integer in DB into a hex string

    :param hash_long: 64bit signed integer
    :type hash_long: int
    :return: Hash string
    :rtype: str or None
    """
...
def hash_str_to_int(hash_str):
    """Convert a hex string into a 64bit signed integer

    :param hash_long: Hash string
    :type hash_long: str
...
def timestamp_ms_to_iso8601(timestamp_ms):
...
def iso8601_to_timestamp(date_time_string):
...
class MetricsRepository(metrics_repository.AbstractMetricsRepository):
    ...
    def _get_metrics(self, tenant_id, region,
        ...
        while rs.has_next():
            rs.get_next(row)
            _hash = hash_int_to_str(row.get_field_as_long(0))
        ...
        for metric_hash, metric in metrics.items():
            ...
            if end_timestamp:
                ...
                filters.append("TO_EPOCH_MS(timestamp) <= %d" %
                               end_timestamp_ms)
            if metrics:
                filters.append("hash = %d" % hash_str_to_int(metric_hash))
        ...
        for query in queries:
            ...
            while rs.has_next():
                ...
                measurement = Measurement(
                    ...
                    hash_int_to_str(row.get_field_as_long(1)),  # hash
    ...
    def measurement_list(self, tenant_id, region, name, dimensions,
        ...
        """
        if offset is not None:
            offset_timestamp = iso8601_to_timestamp(offset)
        ...
        for groups, measurements in measurements_per_groups.items():
            measurements_list = [
                (timestamp_ms_to_iso8601(m.timestamp), m.value, m.value_meta)
                for m in measurements]
    ...
    def metrics_statistics(self, tenant_id, region, name, dimensions,
        ...
        for groups, measurements in measurements_per_groups.items():
            ...
            start_time = timestamp_ms_to_iso8601(m.timestamp)
    ...
    def alarm_history(self, tenant_id, alarm_id_list,
        ...
        for ah in alarm_histories:
            alarm = {u'timestamp': timestamp_ms_to_iso8601(ah.timestamp),
