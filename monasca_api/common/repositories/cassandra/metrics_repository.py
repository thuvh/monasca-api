    ...
    def measurement_list(self, tenant_id, region, name, dimensions,
        ...
        if len(metrics) > 1:
            ...
            if merge_metrics_flag:
                for metric in metrics:
                    query = self._build_measurement_query(metric['id'],
                                                          ...
                                                          offset_timestamp,
                                                          limit)
                    results.append((metric, self.session.execute_async(query[0], query[1]).result()))
            ...
            query = self._build_measurement_query(metrics[0]['id'],
                                                  ...
                                                  offset_timestamp,
                                                  limit)
            results.append((metrics[0], self.session.execute_async(query[0], query[1]).result()))
        ...
        if not merge_metrics_flag:
            ...
            for i in range(0, num_series):
                row = results[i].next()
                top_batch.append([i, [row.time_stamp,
                                      row.value,
                                      rest_utils.from_json(row.value_meta) if row.value_meta else {}]])
            top_batch.sort(key=lambda m: m[1][0], reverse=True)

            while count < limit:
                measurements.append(top_batch[num_series - 1][1])
                count += 1
                row = results[top_batch[num_series - 1][0]].next()
                if not row:
                    num_series -= 1
                else:
                    top_batch[num_series - 1][1] = [[row.time_stamp,
                                                     row.value,
                                                     rest_utils.from_json(row.value_meta) if row.value_meta else {}]]
                    top_batch.sort(key=lambda m: m[1][0], reverse=True)

            measurements = [ [self._isotime_msec(m[0]), m[1]. m[2]]  for m in measurements]
            series_list.append({'name': results[0]['name'],
                                'id': results[0]['id'],
                                'columns': ['timestamp', 'value', 'value_meta'],
                                'measurements': measurements})
            return series_list
