...
class MetricsRepository(metrics_repository.AbstractMetricsRepository):
    ...
    def metrics_statistics(self, tenant_id, region, name, dimensions,
        ...
        for series in series_list:
            ...
            LOG.info('stats for series: %s' % series)
            ...
            if not measurements:
                LOG.info('empty measurements: %s' % series)
                continue

            first_measure = measurements[0]
            stat_start_time = MetricsRepository._parse_time_string(first_measure[0])

            # skip blank intervals at the beginning
            stat_start_time = start_time + timedelta(seconds=((stat_start_time - start_time).seconds / period) * period)
    ...
    def alarm_history(self, tenant_id, alarm_id_list,
        ...
        try:
            ...
            LOG.info('alarm query: %s   params: %s' % (ALARM_HISTORY_CQL % tuple(conditions), params))
