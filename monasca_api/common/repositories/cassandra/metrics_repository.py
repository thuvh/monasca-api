...
class MetricsRepository(metrics_repository.AbstractMetricsRepository):
    ...
    def measurement_list(self, tenant_id, region, name, dimensions,
        ...
        try:
            ...
            LOG.info('group by arg: %s' % group_by)
            if group_by:
    ...
    def _build_measurement_query(self, metric_id, start_timestamp,
        ...
        LOG.info('measurement query: %s   %s' % (MEASUREMENT_LIST_CQL % tuple(conditions), params))

        if fetch_size:
            return SimpleStatement(MEASUREMENT_LIST_CQL % tuple(conditions), fetch_size=fetch_size), params
        else:
            return SimpleStatement(MEASUREMENT_LIST_CQL % tuple(conditions)), params
    ...
    def _group_metrics(metrics, group_by, search_by):
        grouped_metrics = {}
        LOG.info('metric list before group: %s  group %s' % (metrics, group_by))
        for metric in metrics:
            key = ''
            display_dimensions = dict(search_by.items())
            for name in group_by:
                # '_' ensures te key with missing dimension is sorted lower
                LOG.info('dimension dict: %s    key: %s' % (metric['dimensions'], name))
            ...
            LOG.info('grouped metric: %s     key: %s      display dimensions: %s' % (metric, key, display_dimensions))

        grouped_metrics = grouped_metrics.items()
        grouped_metrics.sort(key=lambda x: x[0])
        LOG.info('grouped metrics: %s' % grouped_metrics)
    ...
    def metrics_statistics(self, tenant_id, region, name, dimensions,
        ...
        LOG.info('measurements list: %s' % series_list)
        ...
        for series in series_list:
            ...
            first_measure_time = parser.parse(first_measure[0])

            stat_start_time = first_measure_time
            stats_list = []
            stats_count = 0
            stats_sum = 0
            ...
            for measurement in series['measurements']:
                ...
                time_stamp = parser.parse(measurement[0])
                value = measurement[1]

                if (time_stamp - stat_start_time).seconds >= period:

                    stat = MetricsRepository._create_stat(statistics, first_measure_time, stats_count,
                                                          stats_sum, stats_min, stats_max)

                    stats_list.append(stat)
                    limit -= 1

                    first_measure_time = time_stamp
                    stats_sum = 0
                    stats_count = 0
                    stats_min = value
                    stats_max = value
                    ...
                    while time_stamp >= stat_start_time and limit >= 0:

                        stat = MetricsRepository._create_stat(statistics, stat_start_time)
                        stats_list.append(stat)
                        limit -= 1
                        last_timestamp = self._isotime_msec(stat_start_time)
                        stat_start_time += timedelta(seconds=period)
                ...
                last_timestamp = measurement[0]

            if stats_count:

                stat = MetricsRepository._create_stat(statistics, first_measure_time, stats_count, stats_sum,
                                                      stats_min, stats_max)
                stats_list.append(stat)
                limit -= 1

            statistic = {u'name': name.decode('utf8'),
                         u'id': '_'.join([series['id'], last_timestamp]),
        ...
        LOG.info('statistics list: %s' % json_statistics_list)
    ...
    def _create_stat(statistics, timestamp, stat_count=None, stat_sum=None, stat_min=None, stat_max=None):
        ...
        stat = [timestamp.strftime('%Y-%m-%dT%H:%M:%SZ').decode('utf8')]
