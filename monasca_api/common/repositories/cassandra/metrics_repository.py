...
METRIC_LIST_CQL = ('select metric_name, dimensions, metric_id '
                   'from metrics where %s %s %s %s %s %s %s %s %s')
REGION_EQ = 'region = %s'
...
class MetricsRepository(metrics_repository.AbstractMetricsRepository):
    ...
    def _build_metrics_by_name_query(self, tenant_id, region, name, dimensions, start_time, end_time, dim_offset,
        ...
        LOG.info('metric query: %s, %s' % (METRIC_LIST_CQL % tuple(conditions), params))
