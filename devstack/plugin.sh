...
function install_node_nvm {
    ...
    (source ~/.nvm/nvm.sh >> /dev/null; nvm install 4.0.0; nvm use 4.0.0)
}
...
function install_monasca_grafana {
    ...
    (source ~/.nvm/nvm.sh >> /dev/null; nvm use 4.0.0; npm config set unsafe-perm true)
    (source ~/.nvm/nvm.sh >> /dev/null; nvm use 4.0.0; npm install)
    (source ~/.nvm/nvm.sh >> /dev/null; nvm use 4.0.0; npm install grunt-cli)
    grunt --force
    cd "${MONASCA_BASE}"
