...
function recreate_users_postgresql {
  ...
  for user in "${users[@]}"; do
    sudo -u postgres -i psql -c "CREATE USER $user WITH PASSWORD '$DATABASE_PASSWORD'"
    sudo -u postgres -i psql -c "GRANT ALL PRIVILEGES ON $db TO $user"
    sudo -u postgres -i psql -c "GRANT $dbPermissions ON ALL TABLES IN SCHEMA public TO $user"
  done
}
