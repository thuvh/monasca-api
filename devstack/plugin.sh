...
function install_schema {
    ...
    sudo cp -f "${MONASCA_API_DIR}"/devstack/files/schema/mon_$DATABASE_TYPE.sql /opt/monasca/sqls/mon_$DATABASE_TYPE.sql
    sudo ln -sf /opt/monasca/sqls/mon_$DATABASE_TYPE.sql /opt/monasca/sqls/mon.sql
    ...
    if is_service_enabled mysql; then
        ...
        sudo -u postgres psql -d $databaseName -f /opt/monasca/sqls/mon.sql || echo "Did the schema change? This process will fail on schema changes."
    fi
...
function clean_schema {
    ...
    sudo rm -f /opt/monasca/sqls/mon_$DATABASE_TYPE.sql
...
function install_monasca_api_java {
    ...
    sudo cp -f "${MONASCA_API_DIR}"/monasca-api/devstack/files/monasca-api/api-config.yml /etc/monasca/api-config.yml
...
function install_monasca_api_python {
    ...
    local dbAlarmUrl = `get_database_type_$DATABASE_TYPE://monapi:$DATABASE_PASSWORD@$DATABASE_HOST/mon`
    ...
    sudo sed -e "
      ...
      s|%INFLUXDB_PORT%|8086|g;
      s|%KAFKA_HOST%|$SERVICE_HOST|g;
      s|%MONASCA_API_DB_URL%|$dbAlarmUrl|g;
...
function install_monasca_notification {
    ...
    sudo sed -e "
      ...
      s|%MONASCA_NOTIFICATION_DB_ENGINE%|$dbEngine|g;
      s|%KAFKA_HOST%|$SERVICE_HOST|g;
      s|%ZOOKEEPER_HOST%|$SERVICE_HOST|g;
...
function recreate_users_postgresql {
  local db=$1
  local -n users=$2
  local dbPermissions="SELECT, UPDATE, INSERT, DELETE, REFERENCES, CONNECT"

  for user in "${users[@]}"; do
    sudo -u root sudo -u postgres -i psql -c "CREATE USER $user WITH PASSWORD '$DATABASE_PASSWORD'"
    sudo -u root sudo -u postgres -i psql -c "GRANT ALL PRIVILEGES ON $db TO $user"
    sudo -u root sudo -u postgres -i psql -c "GRANT $dbPermissions ON ALL TABLES IN SCHEMA public TO $user"
  done
}
