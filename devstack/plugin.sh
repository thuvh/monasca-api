...
function install_schema {
    ...
    sudo mkdir -p /opt/monasca-notification/sqls || true

    sudo chmod 0755 /opt/monasca-notification/sqls

    if [[ "${MONASCA_METRICS_DB,,}" == 'influxdb' ]]; then

        sudo cp -f "${MONASCA_BASE}"/monasca-api/devstack/files/schema/influxdb_setup.py /opt/monasca-notification/influxdb_setup.py

        sudo chmod 0750 /opt/monasca-notification/influxdb_setup.py

        sudo chown root:root /opt/monasca-notification/influxdb_setup.py

        sudo /opt/monasca-notification/influxdb_setup.py

    fi

    sudo cp -f "${MONASCA_BASE}"/monasca-api/devstack/files/schema/mon_mysql.sql /opt/monasca-notification/sqls/mon.sql

    sudo chmod 0644 /opt/monasca-notification/sqls/mon.sql

    sudo chown root:root /opt/monasca-notification/sqls/mon.sql

    # must login as root@localhost
    sudo mysql -h "127.0.0.1" -uroot -psecretmysql < /opt/monasca-notification/sqls/mon.sql || echo "Did the schema change? This process will fail on schema changes."

    sudo cp -f "${MONASCA_BASE}"/monasca-api/devstack/files/schema/winchester.sql /opt/monasca-notification/sqls/winchester.sql

    sudo chmod 0644 /opt/monasca-notification/sqls/winchester.sql

    sudo chown root:root /opt/monasca-notification/sqls/winchester.sql

    # must login as root@localhost
    sudo mysql -h "127.0.0.1" -uroot -psecretmysql < /opt/monasca-notification/sqls/winchester.sql || echo "Did the schema change? This process will fail on schema changes."
