...
function install_schema {
    ...
    if [[ ${databaseBackend} == "mysql" ]]; then
        sudo mysql -u$DATABASE_USER -p$DATABASE_PASSWORD -h$DATABASE_HOST < /opt/monasca/sqls/mon.sql || echo "Did the schema change? This process will fail on schema changes."
...
function recreate_users_postgresql {
  ...
  for user in "${users[@]}"; do
    for host in "${hosts[@]}"; do
      sudo -u root sudo -u postgres -i psql -c "CREATE ROLE $user LOGIN PASSWORD 'password'"
