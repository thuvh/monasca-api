...
function configure_monasca_api_python_wsgi {
    ...
    sudo cp $MONASCA_API_DIR/monasca_api/api/wsgi/script.py $MONASCA_API_WSGI_DIR/monasca_api
    sudo cp $PLUGIN_FILES/api-wsgi.template $MONASCA_API_apache_conf

    sudo sed -e "
        s|%PUBLICPORT%|$monasca_api_api_port|g;
        s|%APACHE_NAME%|$APACHE_NAME|g;
        s|%PUBLICWSGI%|$MONASCA_API_WSGI_DIR/monasca_api|g;
        s|%USER%|mon-api|g;
