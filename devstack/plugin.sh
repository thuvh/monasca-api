...
function install_monasca_api_python {
    ...
    sudo cp -f "${MONASCA_API_DIR}"/devstack/files/monasca-api/python/api.conf /etc/monasca/api.conf
    sudo chown mon-api:root /etc/monasca/api.conf
    sudo chmod 0660 /etc/monasca/api.conf
    sudo ln -sf /etc/monasca/api.conf /etc/monasca-api.conf
    ...
    sudo ln -sf /etc/monasca/api-logging.conf /etc/monasca-api-logging.conf

    sudo cp -f "${MONASCA_API_DIR}"/devstack/files/monasca-api/python/api-config.ini /etc/monasca/api-paste.ini
    sudo chown mon-api:root /etc/monasca/api-paste.ini
    sudo chmod 0660 /etc/monasca/api-paste.ini
    sudo ln -sf /etc/monasca/api-config.ini /etc/monasca-api-paste.ini
    ...
    sudo sed -e "
        ...
        s|%KAFKA_HOST%|$SERVICE_HOST|g;
        s|%ADMIN_PASSWORD%|$ADMIN_PASSWORD|g;
    " -i /etc/monasca/api.conf

    sudo sed -e "
        s|%MONASCA_API_SERVICE_HOST%|$MONASCA_API_SERVICE_HOST|g;
        s|%MONASCA_API_SERVICE_PORT%|$MONASCA_API_SERVICE_PORT|g;
        s|%API_WORKERS%|$API_WORKERS|g;
    " -i /etc/monasca/api-paste.ini
...
function clean_monasca_api_python {
    ...
    sudo rm /etc/monasca/api.conf

    sudo rm /etc/monasca-api-logging.conf
    sudo rm /etc/monasca/api-logging.conf

    sudo rm /etc/monasca-api-paste.ini
    sudo rm /etc/monasca/api-paste.ini
