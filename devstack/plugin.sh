...
function install_schema_alarm_database {
  ...
  if is_service_enabled mysql; then
      sudo mysql -u$DATABASE_USER -p$DATABASE_PASSWORD -h$MYSQL_HOST < $MONASCA_SCHEMA_DIR/mon.sql || echo "Did the schema change? This process will fail on schema changes."
  elif is_service_enabled postgresql; then
      sudo -u postgres psql -d $databaseName -f $MONASCA_SCHEMA_DIR/mon.sql || echo "Did the schema change? This process will fail on schema changes."
  fi
