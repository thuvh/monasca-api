...
function install_storm {
    ...
    sudo tar -xzf ${storm_tarball} -C /opt/storm
...
function download_file {
    ...
    local curl_z_flag=""
    if [[ -f ${file} ]]; then
        # If the file exists tell curl to download only if newer version exists.
        curl_z_flag="-z $file"
    fi
    ...
    _safe_permission_operation ${CURL_GET} -L $url -m $timeout --retry 3 --retry-delay 5 -o $file $curl_z_flag
