...
function install_node_nvm {
    ...
    (. "${HOME}"/.bashrc >> /dev/null; nvm install 4.0.0; nvm use 4.0.0)
    set +i
}
...
function install_monasca_grafana {
    ...
    (. "${HOME}"/.bashrc >> /dev/null; nvm use 4.0.0; npm config set unsafe-perm true)
    (. "${HOME}"/.bashrc >> /dev/null; nvm use 4.0.0; npm install)
    (. "${HOME}"/.bashrc >> /dev/null; nvm use 4.0.0; npm install -g grunt-cli)
    (. "${HOME}"/.bashrc >> /dev/null; nvm use 4.0.0; grunt --force)
