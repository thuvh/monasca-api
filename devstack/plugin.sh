...
function create_system_accounts {
    ...
    for user in "${!MONASCA_SYSTEM_USERS[@]}"; do
        ...
        sudo useradd -g "${group}" -G "${STACK_USER}" -s /bin/bash -d /opt/monasca -m "${user}" || true
    done
}

function clean_system_accounts {
    for user in "${!MONASCA_SYSTEM_USERS}"; do
        sudo userdel --force "${user}" || true
    done
    sudo groupdel "${MONASCA_SYSTEM_GROUP}" || true
}
...
function configure_monasca_api_python {
    ...
    sudo chown "${MONASCA_API_USER}":root ${api_config}
    ...
    sudo chown "${MONASCA_API_USER}":root ${api_logging_config}
    ...
    sudo chown "${MONASCA_API_USER}":root ${api_paste_config}
...
function configure_monasca_api_python_wsgi {
    ...
    sudo sed -e "
        ...
        s|%GROUP%|${MONASCA_SYSTEM_GROUP}|g;
