...
MONASCA_NOTIFICATION_CONF=${MONASCA_NOTIFICATION_CONF:-$MONASCA_NOTIFICATION_CONF_DIR/notification.conf}
...
start_monasca-notification(){
    if is_monasca_notification_enabled; then
        ...
        run_process "monasca-notification" "$MONASCA_NOTIFICATION_BIN_DIR/monasca-notification --config-file $MONASCA_NOTIFICATION_CONF"
    fi
}
