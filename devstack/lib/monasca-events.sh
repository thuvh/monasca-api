...
function configure_monasca-events {
    configure_kafka
    create_events_endpoints
    configure_monasca_api
}

function configure_monasca_api {
    echo_summary "Enable events in monasca-api configuration"
    iniset "$MONASCA_API_CONF" DEFAULT enable_events_api "true"
    iniset "$MONASCA_API_CONF" kafka openstac_events "openstack_events"
}
...
function create_events_endpoints {
