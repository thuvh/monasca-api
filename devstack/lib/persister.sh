...
if [[ "${MONASCA_METRICS_DB,,}" == 'cassandra' ]]; then
    MONASCA_PERSISTER_BATCH_SIZE=1
    MONASCA_PERSISTER_MAX_BATCH_TIME=10
    MONASCA_PERSISTER_METRIC_THREADS=20
    MONASCA_PERSISTER_MAX_BATCH_TIME=10000
else
    MONASCA_PERSISTER_BATCH_SIZE=100
    MONASCA_PERSISTER_MAX_BATCH_TIME=15
    MONASCA_PERSISTER_METRIC_THREADS=10
    MONASCA_PERSISTER_MAX_BATCH_TIME=0
fi
...
configure_monasca_persister_python() {
    ...
    iniset "$MONASCA_PERSISTER_CONF" kafka num_processors 2
    iniset "$MONASCA_PERSISTER_CONF" kafka database_batch_size 30
    iniset "$MONASCA_PERSISTER_CONF" kafka commit_batch_time 10000
    ...
    if [[ "${MONASCA_METRICS_DB,,}" == 'influxdb' ]]; then
        ...
        iniset "$MONASCA_PERSISTER_CONF" cassandra max_write_retries 8
        iniset "$MONASCA_PERSISTER_CONF" cassandra max_definition_cache_size 50000000
