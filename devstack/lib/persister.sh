...
configure_monasca_persister_python() {
    ...
    if [[ "${MONASCA_METRICS_DB,,}" == 'influxdb' ]]; then
        ...
        iniset "$MONASCA_PERSISTER_CONF" repositories alarm_state_history_driver ${M_REPO_DRIVER_INFLUX}
    else
        ...
        iniset "$MONASCA_PERSISTER_CONF" repositories metrics_driver ${AH_REPO_DRIVER_CASSANDRA}
