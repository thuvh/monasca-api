...
configure_monasca_persister_python() {
    ...
    install -m 600 /tmp/persister.conf ${MONASCA_PERSISTER_CONF}
    install -m 600 ${MONASCA_PERSISTER_DIR}/etc/monasca/persister-logging.conf ${MONASCA_PERSISTER_LOGGING_CONF}

    install -m 600 /tmp/persister.conf ${MONASCA_PERSISTER_CONF} && rm -rf /tmp/persister.con

    iniset "$MONASCA_PERSISTER_CONF" DEFAULT log_config_append ${MONASCA_PERSISTER_LOGGING_CONF}

    iniset "$MONASCA_PERSISTER_CONF" kafka uri $SERVICE_HOST:9092
    iniset "$MONASCA_PERSISTER_CONF" kafka consumer_id 1
    iniset "$MONASCA_PERSISTER_CONF" kafa client_id 1
    iniset "$MONASCA_PERSISTER_CONF" kafka database_batch_size 1000
    iniset "$MONASCA_PERSISTER_CONF" kafka max_wait_time_secobds 30
    iniset "$MONASCA_PERSISTER_CONF" kafka fetch_size_bytes 4096
    iniset "$MONASCA_PERSISTER_CONF" kafka max_buffer_size 32768
    ...
    iniset "$MONASCA_PERSISTER_CONF" zookeeper partition_interval_recheck_seconds 15
...
Merge branch 'master' of ssh://review.openstack.org:29418/openstack/monasca-api
