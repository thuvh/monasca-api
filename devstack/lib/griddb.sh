...
function install_griddb_client {
    echo_summary "Install GridDB C Client"

    # Install GridDB C client library
    apt_get -y install swig libtool automake autoconf m4
    ${MONASCA_API_DIR}/tools/install_griddb_c_client.sh devstack

    # Install GridDB Python client
    sudo -EH pip install -e git+https://github.com/yosshy/griddb_client.git#egg=griddb_python_cilent
}

function clean_griddb_client {
    sudo rm /usr/local/lib/libgridstore*
}
...
function install_monasca_griddb {
    ...
    # Aquire GridDB database server source code and build it
    ...
    # Create griddb user and its group
    ...
    # Install GridDB database server
