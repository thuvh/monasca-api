...
function is_storm_enabled {
    is_service_enabled monasca-storm && return 0
    return 1
}
...
function start_storm {
    if is_storm_enabled; then
        ...
        run_process "storm-nimbus" "${STORM_NIMBUS_CMD}" "${STORM_GROUP}" "${STORM_USER}"
        run_process "storm-supervisor" "${STORM_SUPERVISOR_CMD}" "${STORM_GROUP}" "${STORM_USER}"
    fi
}
...
function stop_storm {
    if is_storm_enabled; then
        ...
        stop_process "storm-nimbus"
        stop_process "storm-supervisor"
    fi
}
...
function configure_storm {
    if is_storm_enabled; then
        ...
        sudo cp -f "${MONASCA_API_DIR}"/devstack/files/storm.yaml "${STORM_DIR}/current/conf/storm.yaml"
        sudo chown "${STORM_USER}":"${STORM_GROUP}" "${STORM_DIR}/current/conf/storm.yaml"
        sudo chmod 0644 "${STORM_DIR}/current/conf/storm.yaml"
    fi
}
...
function _install_storm {
    ...
    sudo ln -sf "${STORM_DIR}"/apache-storm-${STORM_VERSION} "${STORM_DIR}/current"
}
...
function _create_directories {
    ...
    sudo ln -sf "${STORM_LOG_DIR}" "${STORM_DIR}/current/logs"
}
