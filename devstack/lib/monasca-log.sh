...
KIBANA_DEV_NODE_JS_VERSION=${KIBANA_DEV_NODE_JS_VERSION:-"14.18.1"}
...
function build_kibana_plugin {
    if is_service_enabled kibana; then
        ...
        git pull https://review.opendev.org/openstack/monasca-kibana-plugin refs/changes/79/908779/8
        mv $MONASCA_KIBANA_PLUGIN_DIR $MONASCA_KIBANA_PLUGIN_DIR_v2
        cp -r $MONASCA_KIBANA_PLUGIN_DIR_v2 "$KIBANA_DEV_DIR/plugins"
        local plugin_dir="$KIBANA_DEV_DIR/plugins/monascaKibanaPlugin"
        ...
        local pkg="$plugin_dir/build/monascaKibanaPlugin-$monasca_kibana_plugin_version.zip"
        local easyPkg=$DEST/monascaKibanaPlugin.zip
        ln $pkg $easyPkg
...
function install_kibana_plugin {
    if is_service_enabled kibana; then
        ...
        local pkg=file://$DEST/monascaKibanaPlugin.zip
