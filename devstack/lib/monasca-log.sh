...
function build_kibana_plugin {
    if is_service_enabled kibana; then
        ...
        git pull https://review.opendev.org/openstack/monasca-kibana-plugin refs/changes/79/908779/4
        cp -r $MONASCA_KIBANA_PLUGIN_DIR "$KIBANA_DEV_DIR/plugins"
        cp "$KIBANA_DEV_DIR/plugins/monasca-kibana-plugin/package.json" "$KIBANA_DEV_DIR/plugins/monasca-kibana-plugin/kibana.json"
