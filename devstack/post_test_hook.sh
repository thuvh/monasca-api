...
function setup_monasca_api {
    ...
    pushd $TEMPEST_DIR
    sudo -EH pip install -r requirements.txt -r test-requirements.txt
    sudo -EH pip install nose
    sudo -EH pip install numpy
    popd;

    pushd $MONASCA_API_DIR
    sudo -EH pip install $constraints -U -r requirements.txt -r test-requirements.txt
