...
Monasca supports storing both InfluxDB and Vertica for storing metrics and alarm state history. InfluxDB is enabled by default in the DevStack environment.
...
2. Modify the environment variable `MONASCA_METRICS_DB` in `local.conf` or `settings file` from influxdb to vertica as follows: 
