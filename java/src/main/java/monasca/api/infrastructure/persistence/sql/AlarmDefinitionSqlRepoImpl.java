...
public class AlarmDefinitionSqlRepoImpl
  ...
  public void deleteById(String tenantId, String alarmDefId) {
    ...
    try {
      ...
      AlarmDefinitionDb result = (AlarmDefinitionDb) session
  ...
  public String exists(String tenantId, String name) {
    ...
    try {
      ...
      return CollectionUtils.isEmpty(ids) ? null : (String) ids.get(0);
    } finally {
      if (session != null) {
        session.close();
      }
  ...
  public List<AlarmDefinition> find(String tenantId, String name, Map<String, String> dimensions, String offset, int limit) {
    ...
    try {
      ...
      qAlarmDefinition.setResultTransformer(ClobAwareSQLResultTransformer.INSTANCE);
  ...
  public AlarmDefinition findById(String tenantId, String alarmDefId) {
    Session session = null;
    List<String> okActionIds = Lists.newArrayList();
    List<String> alarmActionIds = Lists.newArrayList();
    List<String> undeterminedActionIds = Lists.newArrayList();

    try {
      session = sessionFactory.openSession();

      AlarmDefinitionDb alarmDefinitionDb = (AlarmDefinitionDb) session
          .getNamedQuery(AlarmDefinitionDb.Queries.FIND_BY_TENANT_AND_ID_NOT_DELETED)
          .setString("tenant_id", tenantId)
          .setString("id", alarmDefId).uniqueResult();

      List<AlarmActionDb> alarmActionList = session
  ...
  public void update(String tenantId, String id, boolean patch, String name, String description, String expression, List<String> matchBy,
    ...
    try {
      ...
      this.updateChangedAlarms(changedSubAlarms, session);
  ...
  private void updateChangedAlarms(final Map<String, AlarmSubExpression> changedSubAlarms,
                                   final Session session) {
