...
import monasca.api.infrastructure.persistence.sql.HibernateUtils;
...
public class InfluxV9AlarmStateHistoryHibernateRepo implements AlarmStateHistoryRepo {

  private static final Logger logger = LoggerFactory.getLogger(InfluxV9AlarmStateHistoryHibernateRepo.class);
  ...
  private final SimpleDateFormat oldSimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssX");

  private static final TypeReference<List<MetricDefinition>> METRICS_TYPE = new TypeReference<List<MetricDefinition>>() {};

  private static final TypeReference<List<AlarmTransitionSubAlarm>> SUB_ALARMS_TYPE = new TypeReference<List<AlarmTransitionSubAlarm>>() {};

  @Inject
  public InfluxV9AlarmStateHistoryHibernateRepo(HibernateUtils hibernateUtils, ApiConfig config, InfluxV9RepoReader influxV9RepoReader,
  ...
  public List<AlarmStateHistory> findById(String tenantId, String alarmId, String offset, int limit) throws Exception {
    ...
    String q =
        String.format("select alarm_id, metrics, old_state, new_state, " + "reason, reason_data, sub_alarms " + "from alarm_state_history "
            + "where %1$s %2$s %3$s %4$s", this.influxV9Utils.publicTenantIdPart(tenantId), this.influxV9Utils.alarmIdPart(alarmId),
            this.influxV9Utils.timeOffsetPart(offset), this.influxV9Utils.limitPart(limit));
  ...
  @Override
  public List<AlarmStateHistory> find(String tenantId, Map<String, String> dimensions, DateTime startTime, @Nullable DateTime endTime,
      @Nullable String offset, int limit) throws Exception {

    List<String> alarmIdList = this.hibernateUtils.findAlarmIds(tenantId, dimensions);
    ...
    String q =
        String.format("select alarm_id, metrics, old_state, new_state, " + "reason, reason_data, sub_alarms " + "from alarm_state_history "
            + "where %1$s %2$s %3$s %4$s %5$s", this.influxV9Utils.publicTenantIdPart(tenantId),
            this.influxV9Utils.startTimeEndTimePart(startTime, endTime), this.influxV9Utils.alarmIdsPart(alarmIdList),
            this.influxV9Utils.timeOffsetPart(offset), this.influxV9Utils.limitPart(limit));
