...
public class MeasurementVerticaRepoImpl implements MeasurementRepo {
  ...
  private static final String FIND_BY_METRIC_DEF_SQL =
      "select def.name, mes.definition_dimensions_id, defdim.dimension_set_id, defdim.definition_id, "
      + "mes.id, mes.time_stamp, mes.value, mes.value_meta "
      + "from MonMetrics.Measurements mes, MonMetrics.Definitions def, MonMetrics.DefinitionDimensions defdim "
      + "%s "
      + "where mes.definition_dimensions_id = defdim.id "
      + "and def.id = defdim.definition_id "
  ...
  public List<Measurements> find(
    ...
    try (Handle h = db.open()) {
      ...
      for (Map<String, Object> row : rows) {
        ...
        byte[] defDimIdBytes = (byte[]) row.get("definition_dimensions_id");

        byte[] dimSetIdBytes = (byte[]) row.get("dimension_set_id");

        ByteBuffer defDimId = ByteBuffer.wrap(defDimIdBytes);

        // Throw away measurement id.
        ...
        if (Boolean.TRUE.equals(mergeMetricsFlag)) {
          ...
          measurements = results.get(defDimId);
        ...
        if (measurements == null) {
          ...
          if (Boolean.TRUE.equals(mergeMetricsFlag)) {
            ...
            results.put(defDimId, measurements);
