...
public class NotificationMethodMySqlRepoImpl implements NotificationMethodRepo {
  ...
  public NotificationMethod create(String tenantId, String name,
    ...
    try (Handle h = db.open()) {
      ...
      h.insert(
          "insert into notification_method (id, tenant_id, name, type, address, periodic_interval, created_at, updated_at) values (?, ?, ?, ?, ?, ?, NOW(), NOW())",
          id, tenantId, name, type.toString(), address, periodicInterval);
  ...
  public List<NotificationMethod> find(String tenantId, List<String> sortBy, String offset,
    ...
    try (Handle h = db.open()) {
      ...
      String rawQuery =
          "  SELECT nm.id, nm.tenant_id, nm.name, nm.type, nm.address, nm.periodic_interval, nm.created_at, nm.updated_at "
  ...
  public NotificationMethod update(String tenantId, String notificationMethodId, String name,
    ...
    try (Handle h = db.open()) {
      ...
      if (h
          .update(
              "update notification_method set name = ?, type = ?, address = ?, periodic_interval = ?, updated_at = NOW() "
              + "where tenant_id = ? and id = ?",
              name, type.name(), address, periodicInterval, tenantId, notificationMethodId) == 0)
        throw new EntityNotFoundException("No notification method exists for %s",
            notificationMethodId);
      h.commit();
