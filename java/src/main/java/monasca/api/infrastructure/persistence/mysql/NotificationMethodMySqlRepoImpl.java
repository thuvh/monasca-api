...
public class NotificationMethodMySqlRepoImpl implements NotificationMethodRepo {
  ...
  public NotificationMethod create(String tenantId, String name,
      String pNotificaitonMethodType, String address, int period) {
    try (Handle h = db.open()) {
      ...
      h.insert(
          ...
          id, tenantId, name, pNotificaitonMethodType, address, period);
      LOG.debug("Creating notification method {} for {}", name, tenantId);
      h.commit();
      return new NotificationMethod(id, name, pNotificaitonMethodType, address, period);
    }
  }
    ...
    try (Handle h = db.open()) {
      ...
      if (h
          .update(
              ...
              name, pNotificaitonMethodType, address, period, tenantId, notificationMethodId) == 0)
        throw new EntityNotFoundException("No notification method exists for %s",
            notificationMethodId);
      h.commit();
      return new NotificationMethod(notificationMethodId, name, pNotificaitonMethodType, address, period);
    }
  }
}
