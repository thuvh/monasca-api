...
public class InfluxV9Utils {
  ...
  /**
   * Limits the number of points returned.
   *
   * @param limit
   * @return  limit clause for points
   */
  ...
  /**
   * Limits the number of series returned.
   * @param limit
   * @return  limit clause for series
   */
  public String sLimitPart(int limit) {

    // We add 1 to limit to determine if we need to insert a next link.
    return String.format(" slimit %1$d", limit + 1);
  }

  /**
   * Offset for points.
   * @param startIndex
   * @return  offset clause for points
   */
   ...
   * Offset for series.
   * @param startIndex
   * @return  offset clause for series
   */
  public String sOffsetPart(int startIndex) {

    return String.format(" soffset %1$d", startIndex);
  }
  ...
  public String periodPart(int period) {
    ...
    return period >= 1 ? String.format(" group by time(%1$ds), *", period) : " group by *";
  }
