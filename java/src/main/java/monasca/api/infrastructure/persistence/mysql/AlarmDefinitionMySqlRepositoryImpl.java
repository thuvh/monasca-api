...
public class AlarmDefinitionMySqlRepositoryImpl implements AlarmDefinitionRepository {
  ...
  public AlarmDefinition findById(String tenantId, String alarmDefId) {
    ...
    try (Handle h = db.open()) {
      String query = "select alarm_definition.*,"
          + "group_concat(alarm_action.action_id) as notificationIds,group_concat(alarm_action.alarm_state) as states "
          + "from alarm_definition, alarm_action where alarm_definition.id=alarm_action.alarm_definition_id "
          + "and alarm_definition.tenant_id=:tenantId and alarm_definition.id=:alarmDefId and alarm_definition.deleted_at is NULL group by alarm_definition.id";
