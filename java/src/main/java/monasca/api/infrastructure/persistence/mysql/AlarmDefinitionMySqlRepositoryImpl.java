  ...
  public List<AlarmDefinition> find(String tenantId, String name,
    ...
    try (Handle h = db.open()) {
      String query = "SELECT T1.*, "
          ...
          + " AND deleted_at IS NULL %s ORDER BY %s alarm_definition.created_at %s) AS T1 LEFT OUTER JOIN alarm_action ON T1.id=alarm_action.alarm_definition_id"
          + " GROUP BY T1.id";
  ...
  public AlarmDefinition findById(String tenantId, String alarmDefId) {
    ...
    try (Handle h = db.open()) {
      String query = "SELECT alarm_definition.*,"
          ...
          + " WHERE alarm_definition.tenant_id=:tenantId AND alarm_definition.id=:alarmDefId AND alarm_definition.deleted_at IS NULL GROUP BY alarm_definition.id";
