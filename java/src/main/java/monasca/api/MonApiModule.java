...
public class MonApiModule extends AbstractModule {
  ...
  public SessionFactory getSessionFactory() {
    try {
      ...
      configuration.setProperties(this.getORMProperties(config.hibernate.getDataSourceClassName()));
  ...
  private Properties getORMProperties(final String dataSourceClassName) {
    ...
    switch(dataSourceClassName){
      ...
      case "com.mysql.jdbc.jdbc2.optional.MysqlDataSource":
        properties.put("hibernate.hikari.dataSource.url", config.hibernate.getDataSourceUrl());
        break;
    }
    // different drivers requires different sets of properties
    ...
    properties.put("show_sql", config.getLoggingFactory().getLevel().toString().equalsIgnoreCase("debug"));
