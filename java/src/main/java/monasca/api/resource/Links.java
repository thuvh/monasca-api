  ...
  public static Object paginate(int limit, List<? extends AbstractEntity> elements, UriInfo uriInfo) {
    ...
    if (elements != null) {
      if (elements.size() > limit) {
        ...
        nextLink.rel = "next";
        // Create a new URL with the new offset. Offset is from the second to last element.
        nextLink.href =
            uriInfo.getAbsolutePath().toString() + "?offset=" + elements.get(elements.size() - 2)
                .getId();
        ...
        // Pop the extra element off the list.
