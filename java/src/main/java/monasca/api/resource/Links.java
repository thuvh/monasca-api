...
public final class Links {
  ...
  public static Paged paginateDimensionNames(DimensionNames dimNames, int limit, UriInfo uriInfo)
    ...
    if ((null != dimNames) && (dimNames.getNames().size() > limit)) {
      dimNames.getNames().remove(dimNames.getNames().size()-1);
      String offset = dimNames.getNames().get(dimNames.getNames().size()-1);
      paged.links.add(getNextLink(offset, uriInfo));
    }
