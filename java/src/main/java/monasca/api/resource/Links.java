...
public final class Links {
  ...
  public static Paged paginateDimensionNamesOrValues(DimensionNames dimNamesOrValues, int limit, UriInfo uriInfo)
          throws UnsupportedEncodingException {
    Paged paged = new Paged();
    List<DimensionNames> elements = new ArrayList<>();
    paged.links.add(getSelfLink(uriInfo));

    if ((dimNamesOrValues != null) && (dimNamesOrValues.getDimensionInfo().size() > limit)) {
      dimNamesOrValues.getDimensionInfo().remove(dimNamesOrValues.getDimensionInfo().size()-1);
      String offset = dimNamesOrValues.getDimensionInfo().get(dimNamesOrValues.getDimensionInfo().size()-1);
      paged.links.add(getNextLink(offset, uriInfo));
    }

    elements.add(dimNamesOrValues);
    paged.elements = elements;
    return paged;
  }
