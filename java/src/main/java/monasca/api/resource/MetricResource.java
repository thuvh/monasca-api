...
public class MetricResource {
  ...
  private final String monitoring_admin_role;
  ...
  public MetricResource(ApiConfig config, MetricService service, MetricDefinitionRepo metricRepo,
    ...
    this.monitoring_admin_role = (config.middleware == null || config.middleware.adminRole == null)
  ...
  public void create(@Context UriInfo uriInfo, @HeaderParam("X-Tenant-Id") String tenantId,
    ...
    for (CreateMetricCommand command : commands) {
      if (!isDelegate) {
        ...
        if (isCrossProject(crossTenantId, tenantId)) {
                           ...
                           @QueryParam("tenant_id") String crossTenantId) throws Exception 
  {
    ...
    String queryTenantId = getQueryProject(roles, crossTenantId, tenantId);

    return Links.paginate(this.persistUtils.getLimit(limit),
                          metricRepo.find(queryTenantId,
                                          name,
                                          dimensions,
                                          offset,
   ...
   */
  private String getQueryProject(String roles,
                                 ...
                                 String tenantId) throws Exception
  {
    ...
    boolean isAdmin = !Strings.isNullOrEmpty(roles) &&
                      COMMA_SPLITTER.splitToList(roles).contains(monitoring_admin_role);
   ...
   */
  private boolean isCrossProject(String crossTenantId, String tenantId) {
