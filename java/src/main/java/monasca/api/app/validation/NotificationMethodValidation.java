...
public class NotificationMethodValidation {
    ...
    private static final List<Integer> VALID_PERIODIC_INTERVALS = Arrays.asList(0, 1);
        ...
        switch (type) {
            case EMAIL : {
                ...
                if (convertedPeriodicInterval != 0)
                    throw Exceptions.unprocessableEntity("Periodic interval can not be non zero for Email");
            } break;
            case WEBHOOK : {
            ...
            case PAGERDUTY : {
                if (convertedPeriodicInterval != 0)
                    throw Exceptions.unprocessableEntity("Periodic interval can not be non zero for Pagerduty");
            } break;
        }
