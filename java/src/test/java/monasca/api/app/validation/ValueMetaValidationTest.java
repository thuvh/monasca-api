...
public class ValueMetaValidationTest {
  ...
  public void maxOk() {
    ...
    for (int i = 0; i < 16; i++) {
      //
      // All 16 name/value pairs (converted to json) must fit in 1024
      // chars.  Test that we can fit 1/16th of 1024 in each pair (64 chars):
      //
      // {"name":"value"}, ...
      // ^^    ^^^     ^^^     <-- extra chars (8 per pair)
      //
      valueMeta.put(makeString(i, 10), makeString(i, (64 - (10+8))));
    }
