...
public class ValueMetaValidationTest {
  ...
  public void maxOk() {
    ...
    for (int i = 0; i < 16; i++) {
      //
      // All 16 name/value pairs (converted to json) must fit in 512
      // chars.  Test that we can fit 1/16th of 512 in each pair (32 chars):
      //
      // {"name":"value"}, ...
      // ^^    ^^^     ^^^     <-- extra chars (8 per pair)
      //
      valueMeta.put(makeString(i, 10), makeString(i, (32 - (10+8))));
    }
