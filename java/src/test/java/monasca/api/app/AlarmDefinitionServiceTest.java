...
public class AlarmDefinitionServiceTest {
  ...
  public void testPatchExistingAlarmName() {
    ...
    public void testUpdateExistingAlarmName() {
        ...
        List<String> undeterminedActions = Arrays.asList("3");
        List<String> matchBy = Arrays.asList("service", "instance_id");
        AlarmDefinition firstAlarmDef = new AlarmDefinition("123", "91% CPU",
                "description1", "LOW", exprStr, matchBy, true, alarmActions,
                okActions, undeterminedActions);

        AlarmDefinition secondAlarmDef = new AlarmDefinition("234", "92% CPU",
                "description2", "LOW", exprStr, matchBy, true, alarmActions,
                okActions, undeterminedActions);

        UpdateAlarmDefinitionCommand updateCommand = new UpdateAlarmDefinitionCommand(
                "92% CPU", "Description1", exprStr, matchBy, "LOW", true,
                alarmActions, okActions, undeterminedActions);

        when(repo.findById(TENANT_ID, secondAlarmDef.getId())).thenReturn(
                secondAlarmDef);
        when(repo.findById(TENANT_ID, firstAlarmDef.getId())).thenReturn(
                firstAlarmDef);
        when(repo.exists(TENANT_ID, "92% CPU")).thenReturn(
                secondAlarmDef.getId());
        ...
        when(notificationMethodRepo.exists(eq(TENANT_ID), anyString()))
                .thenReturn(true);
        AlarmDefinition alarmPatched = service.update(TENANT_ID,
                secondAlarmDef.getId(), alarmExpression, updateCommand);
        assertEquals(alarmPatched.getName(), "92% CPU");
