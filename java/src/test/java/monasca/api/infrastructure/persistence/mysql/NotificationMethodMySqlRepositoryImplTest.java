...
public class NotificationMethodMySqlRepositoryImplTest {
  ...
  public void shouldCreateWebhookNonZeroPeriodicInterval() {
    NotificationMethod nmA = repo.create("555", "MyWebhook", NotificationMethodType.WEBHOOK, "http://localhost:33", 1);
    NotificationMethod nmB = repo.findById("555", nmA.getId());
  ...
  public void shouldUpdateWebhookWithNonZeroPeriodicInterval() {
    NotificationMethod nmOriginal = repo.create("555", "MyWebhook", NotificationMethodType.WEBHOOK, "http://localhost:33", 0);
    repo.update("555", nmOriginal.getId(), "MyWebhook", NotificationMethodType.WEBHOOK, "http://localhost:33", 1);
    NotificationMethod nmUpdated = repo.findById("555", nmOriginal.getId());

    assertEquals(nmUpdated.getPeriodicInterval(), 1);
  }
