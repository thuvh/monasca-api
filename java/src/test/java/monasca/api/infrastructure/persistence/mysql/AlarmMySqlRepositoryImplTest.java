...
public class AlarmMySqlRepositoryImplTest {
  ...
  protected void beforeMethod() {
    ...
    handle
        .execute("insert into alarm (id, alarm_definition_id, state, created_at, updated_at, state_updated_at) values ('1', '1', 'OK', '"+timestamp1.toString().replace('Z', ' ')+"', '"+timestamp1.toString().replace('Z', ' ')+"', '"+timestamp1.toString().replace('Z', ' ')+"')");
    handle
        .execute("insert into alarm (id, alarm_definition_id, state, created_at, updated_at, state_updated_at) values ('2', '1', 'UNDETERMINED', '"+timestamp2.toString().replace('Z', ' ')+"', '"+timestamp2.toString().replace('Z', ' ')+"', '"+timestamp2.toString().replace('Z', ' ')+"')");
    handle
        .execute("insert into alarm (id, alarm_definition_id, state, created_at, updated_at, state_updated_at) values ('3', '1', 'ALARM', '"+timestamp3.toString().replace('Z', ' ')+"', '"+timestamp3.toString().replace('Z', ' ')+"', '"+timestamp3.toString().replace('Z', ' ')+"')");
    long subAlarmId = 42;
    ...
    compoundAlarm =
        new Alarm("234111", "234", "50% CPU", "LOW", buildAlarmMetrics(
            ...
            buildMetricDefinition("cpu.idle_perc", "service", "monitoring", "hostname", "roland",
                "region", "colorado", "extra", "vivi")), AlarmState.UNDETERMINED, "OPEN", "http://somesite.com/this-alarm-info",
  ...
  public void shouldUpdateState() throws InterruptedException {
    ...
    Thread.sleep(1000);
    final Alarm unchangedAlarm = repo.update(TENANT_ID, ALARM_ID, AlarmState.OK, null, null);
  ...
  public void shouldUpdate() throws InterruptedException {
    ...
    Thread.sleep(1000);
    final Alarm newAlarm = repo.update(TENANT_ID, ALARM_ID, AlarmState.UNDETERMINED, "CLOSED", null);
    final DateTime newUpdatedAt = getAlarmUpdatedDate(ALARM_ID);
    assertNotEquals(newUpdatedAt.getMillis(), originalUpdatedAt.getMillis(),
                    "updated_at did not change");
    ...
    newAlarm.setLifecycleState("CLOSED");
    ...
    Thread.sleep(1000);
    final Alarm unchangedAlarm = repo.update(TENANT_ID, ALARM_ID, AlarmState.UNDETERMINED, null, null);
    assertTrue(getAlarmUpdatedDate(ALARM_ID).equals(newUpdatedAt), "updated_at did change");
    assertEquals(unchangedAlarm, newAlarm);
  }
