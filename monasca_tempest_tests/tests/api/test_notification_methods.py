...
class TestNotificationMethods(base.BaseMonascaTest):
    ...
    @test.attr(type="gate")
    def test_patch_notification_method_type_and_period(self):
        type = 'WEBHOOK'
        notification = helpers.create_notification(type=type, period=60)
        resp, response_body = self.monasca_client.create_notifications(
            notification)
        self.assertEqual(201, resp.status)
        self.assertEqual(type, response_body['type'])
        ...
        resp, response_body = \
            self.monasca_client.\
            patch_notification_method(id, type=new_type, period=new_period)
        self.assertEqual(200, resp.status)
        self.assertEqual(new_type, response_body['type'])
        resp, response_body = self.monasca_client.\
            delete_notification_method(id)
