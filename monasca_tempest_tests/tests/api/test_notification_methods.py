...
class TestNotificationMethods(base.BaseMonascaTest):
    ...
    def test_patch_notification_method_name(self):
        name = data_utils.rand_name('notification-')
        notification = helpers.create_notification(name=name)
        resp, response_body = self.monasca_client.patch_notification_method(
            notification)
        self.assertEqual(201, resp.status)
        self.assertEqual(name, response_body['name'])
        id = response_body['id']
    ...
    @test.attr(type="gate")
    @test.attr(type=['negative'])
    def test_patch_notification_method_name_already_exists(self):
        name = data_utils.rand_name('notification-')
        notification = helpers.create_notification(name=name)
        notification1 = helpers.create_notification(name="notification to be changed")
        resp, response_body = self.monasca_client.create_notifications(
            notification)
        id = response_body['id']
        self.assertEqual(201, resp.status)
        ...
        self.assertRaises(exceptions.AlreadyExistsException,
                          self.monasca_client.patch_notification_method, id2, name=name)
        resp, response_body = \
            self.monasca_client.delete_notification_method(id)
        self.assertEqual(204, resp.status)
        resp, response_body = \
            self.monasca_client.delete_notification_method(id2)
        self.assertEqual(204, resp.status)

    @test.attr(type="gate")
    @test.attr(type=['negative'])
    def test_update_notification_method_name_already_exists(self):
        name = data_utils.rand_name('notification-')
        notification = helpers.create_notification(name=name)
        notification1 = helpers.create_notification(name="notification to be changed")
        resp, response_body = self.monasca_client.create_notifications(
        ...
        self.assertEqual(201, resp.status)
        self.assertRaises(exceptions.AlreadyExistsException,
                          self.monasca_client.update_notification_method, id2, name=name, type=response_body['type'],
                          address=response_body['address'], period=response_body['period'])
        resp, response_body = \
            self.monasca_client.delete_notification_method(id)
        self.assertEqual(204, resp.status)
