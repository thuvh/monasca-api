...
class TestNotificationMethods(base.BaseMonascaTest):
    ...
    @test.attr(type="gate")
    @test.attr(type=['negative'])
    def test_create_notification_method_with_invalid_chars_in_name(self):
        for invalid_char in constants.INVALID_CHARS_NOTIFICATION:
            notification = helpers.create_notification(name=invalid_char)
    ...
    @test.attr(type="gate")
    @test.attr(type=['negative'])
    def test_update_notification_method_with_invalid_chars_in_name(self):
        name = data_utils.rand_name('notification-')
        notification = helpers.create_notification(name=name)
        for invalid_char in constants.INVALID_CHARS_NOTIFICATION:
            resp, response_body = self.monasca_client.create_notifications(
            ...
            id = response_body['id']
            self.assertEqual(201, resp.status)
            new_name = invalid_char
            self.assertRaises(exceptions.Conflict,
                              self.monasca_client.update_notification_method,
                              id, name=new_name, type=response_body['type'],
                              address=response_body['address'])
            resp, response_body = \
                self.monasca_client.delete_notification_method(id)
            self.assertEqual(204, resp.status)
