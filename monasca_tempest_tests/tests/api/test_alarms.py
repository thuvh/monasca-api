...
class TestAlarms(base.BaseMonascaTest):
    ...
    def test_list_alarms(self):
        alarm_definition_ids, test_name, test_key, test_value \
        ...
        for element in response_body['elements']:
            ...
            if metric['name'] == test_name:
                self._verify_metric_in_alarm(metric, test_name, test_key,
                                             test_value)
                return
    ...
    def test_list_alarms_by_alarm_definition_id(self):
        ...
        metric = element['metrics'][0]
        self._verify_metric_in_alarm(metric, test_name, test_key, test_value)
    ...
    def test_list_alarms_by_metric_name(self):
        ...
        self._verify_list_alarms_elements(resp, response_body,
                                          expect_num_elements=1)
        query_parms = '?metric_name=' + str(test_name)
    ...
    def test_list_alarms_by_metric_dimensions(self):
        ...
        self._verify_list_alarms_elements(resp, response_body,
                                          expect_num_elements=1)
        query_parms = '?metric_dimensions=' + test_key + ':' + test_value
        resp, response_body = self.monasca_client.list_alarms(query_parms)
        self.assertEqual(200, resp.status)
        self.assertEqual(200, resp.status)
        element = response_body['elements'][0]
    ...
    def test_list_alarms_by_lifecycle_state(self):
        ...
        resp, response_body = self.monasca_client.patch_alarm(
            id=alarm_id, lifecycle_state="OPEN")
    ...
    def _create_alarms_for_test_alarms(self, num):
        ...
        for j in xrange(num):
            for i in xrange(60):
                ...
                if len(elements) >= 1:
                    break
        return alarm_definition_ids, metric_name, key, value
    ...
    def _verify_metric_in_alarm(self, metric, test_name, test_key, test_value):
        self.assertEqual(metric['dimensions'][test_key], test_value)
