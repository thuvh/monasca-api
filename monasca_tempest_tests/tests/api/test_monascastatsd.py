...
class TestMonascaStatsd(base.BaseMonascaTest):
    ...
    def test_basic_statsd(self):
        ...
        gauge_name = statsd.get_gauge(name='TestGauge')
        gauge_name.send('TestMetric', 1, dimensions={'A:B': '1', 'BC': '3'})
        for timer in xrange(constants.MAX_RETRIES):
            query_params = '?name=monasca.TestGauge.TestMetric'
            resp = self.monasca_client.list_metrics(query_params)
            if len(resp) != 0:
                return
            else:
