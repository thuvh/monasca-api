...
class TestAlarmDefinitions(base.BaseMonascaTest):
    ...
    def test_list_alarm_definitions(self):
        ...
        alarm_def_ids, res_body_create_alarm_def = \
            self._create_alarm_definitions_with_num(expression=expression,
                                                    num=1)
        query_param = '?name=' + str(res_body_create_alarm_def['name'])
        ...
        self._verify_list_alarm_definitions_elements(elements, 1,
    ...
    def test_get_alarm_definition(self):
        alarm_def_ids, response_body \
            = self._create_alarm_definitions_with_num(expression=None, num=1)
    ...
    def test_patch_alarm_definition_with_a_different_match_by(self):
        ...
        self.assertRaises(exceptions.UnprocessableEntity,
                          ...
                          id=alarm_def_ids[0], match_by=patched_match_by)
    ...
    def test_patch_actions_in_alarm_definition(self):
        ...
        # Create an alarm definition
        alarm_def_ids, response_body = self._create_alarm_definitions_with_num(
            expression=None, num=1)
        # Patch alarm definition
    ...
    def _create_alarm_definitions_with_num(self, expression, num):
        ...
        if num == 1:
            return alarm_def_ids, response_body
        else:
            return alarm_def_ids
    ...
    def _verify_list_alarm_definitions_elements(self, elements, num,
                                                res_body_create_alarm_def):
        self.assertEqual(len(elements), num)
        for element in elements:
            self._verify_element_set(element)
            self.assertEqual(element['name'],
                             res_body_create_alarm_def['name'])
            self.assertEqual(element['expression'],
                             res_body_create_alarm_def['expression'])
            self.assertEqual(element['id'], res_body_create_alarm_def['id'])
            self.assertEqual(element['description'],
                             res_body_create_alarm_def['description'])
            self.assertEqual(element['match_by'],
                             res_body_create_alarm_def['match_by'])
            self.assertEqual(element['severity'],
