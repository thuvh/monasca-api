...
class TestAlarmTransitions(base.BaseMonascaTest):
    ...
    def test_alarm_max_function(self):
        ...
        for x in xrange(WAIT_SECS):
            resp, resp_body = self.monasca_client.list_alarms(query_params="?alarm_definition_id=" + definition_id)
        ...
        for x in xrange(WAIT_SECS):
            ...
            self.assertEqual(200, resp.status)
            if resp_body['state'] != "ALARM":
                time.sleep(1)
                continue
        self.assertEqual("ALARM", resp_body['state'])
    ...
    def test_alarm_last_function(self):
        ...
        for x in xrange(WAIT_SECS):
            resp, resp_body = self.monasca_client.get_alarm(alarm_id)
            self.assertEqual(200, resp.status)
            if resp_body['state'] != "OK":
                time.sleep(1)
                continue
        self.assertEqual("OK", resp_body['state'])
