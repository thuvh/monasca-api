...
class TestMetrics(base.BaseMonascaTest):
    ...
    def resource_setup(cls):
        ...
        time_iso_millisecond = helpers.timestamp_to_iso_millis(timestamp)
        metric_value = 1.23
        ...
        time_iso_millisecond2 = helpers.timestamp_to_iso_millis(timestamp2)
        metric_value2 = 4.56
        ...
        cls._test_time_iso_millisecond = [time_iso_millisecond,
                                          time_iso_millisecond2]
        cls._test_metric_value = [metric_value, metric_value2]
        cls._test_meta_key = [meta_key, meta_key2]
        cls._test_meta_value = [meta_value, meta_value2]
    ...
    def test_create_metric(self):
        ...
        query_param = '?name=' + self._test_name + \
                      '&start_time=' + self._time_iso
    ...
    def test_create_metrics(self):
        ...
        self.assertEqual(204, resp.status)
        query_param = '?name=' + self._test_name + '&start_time=' + \
                      str(self._time_iso)
        for i in xrange(constants.MAX_RETRIES):
            resp, response_body = self.monasca_client.\
                list_measurements(query_param)
            ...
            for element in elements:
                if str(element['name']) == self._test_name \
                    ...
                    self.assertEqual(str(element['dimensions']
                                     ...
                                     self._test_value)
                    self.assertEqual(str(element['id']),
                                     self._test_time_iso_millisecond[1])
                    ...
                    self.assertEqual(str(first_measurement[0]),
                                     self._test_time_iso_millisecond[0])
                    self.assertEqual(first_measurement[1],
                                     self._test_metric_value[0])
                    self.assertEqual(str(first_measurement[2]
                                         [self._test_meta_key[0]]),
                                     self._test_meta_value[0])
                    self.assertEqual(str(second_measurement[0]),
                                     self._test_time_iso_millisecond[1])
                    self.assertEqual(second_measurement[1],
                                     self._test_metric_value[1])
                    self.assertEqual(str(second_measurement[2]
                                         [self._test_meta_key[1]]),
                                     self._test_meta_value[1])
                    return
            time.sleep(constants.RETRY_WAIT_SECS)
            if i == constants.MAX_RETRIES - 1:
