    ...
    def test_list_metrics_with_dimensions(self):
        ...
        if timer == WAIT_TIME - 1:
            skip_msg = "Skipped test_list_metrics_with_dimensions: timeout " \
                       "on waiting for metrics: at least one metric is " \
                       "needed. Current number of metrics = 0"
    ...
    def test_list_metrics_with_name(self):
        ...
        if timer == WAIT_TIME - 1:
            skip_msg = "Skipped test_list_metrics_with_name: timeout on " \
                       "waiting for metrics: at least one metric is " \
                       "needed. Current number of metrics = 0"
            raise self.skipException(skip_msg)
    ...
    def test_list_metrics_with_offset_limit(self):
        ...
        if timer == WAIT_TIME - 1:
            skip_msg = ("Skipped test_list_metrics_with_offset_limit: "
                        "timeout on waiting for metrics: 4 metrics are "
                        "needed. Current number of "
                        "elements = {}").format(len(elements))
            raise self.skipException(skip_msg)
