...
class TestMetrics(base.BaseMonascaTest):
    ...
    @test.attr(type='gate')
    @test.attr(type=['negative'])
    def test_create_metric_with_value_meta_key_exceeds_max_length(self):
        long_value_meta_key = "x" * (constants.MAX_VALUE_META_VALUE_LENGTH + 1)
        value_meta_value = "y"
        metric = helpers.create_metric(name='name',
                                       value_meta=
                                       {long_value_meta_key: value_meta_value}
                                       )
        self.assertRaises(exceptions.UnprocessableEntity,
                          self.monasca_client.create_metrics,
                          metric)
    ...
    def test_create_metric_with_value_meta_exceeds_max_length(self):
        ...
        metric = helpers.create_metric(name='name',
                                       value_meta=
                                       {value_meta_key: long_value_meta_value}
                                       )
        self.assertRaises(exceptions.UnprocessableEntity,
                          self.monasca_client.create_metrics,
                          metric)
