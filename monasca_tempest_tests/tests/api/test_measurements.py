...
class TestMeasurements(base.BaseMonascaTest):
    ...
    def resource_setup(cls):
        ...
        metric2 = helpers.create_metric(name=name1,
        ...
        metric3 = [
            helpers.create_metric(
                name=name2, timestamp=start_timestamp + 1000 * 2,
                dimensions={'key1': 'value1', 'key2': 'value1'}),
            helpers.create_metric(
                name=name2, timestamp=start_timestamp + 1000 * 2 + 1,
                dimensions={'key1': 'value2', 'key2': 'value2'}),
            helpers.create_metric(
                name=name2, timestamp=start_timestamp + 1000 * 2 + 2,
                dimensions={'key1': 'value3', 'key2': 'value3'}),
            helpers.create_metric(
                name=name2, timestamp=start_timestamp + 1000 * 2 + 3,
        ...
        metric4 = helpers.create_metric(
            name=name1, timestamp=start_timestamp + 1000 * 3,
            dimensions={'key-1': 'value-1'},
            value=NUM_MEASUREMENTS + 1)
        cls.monasca_client.create_metrics(metric4)

        end_time = str(helpers.timestamp_to_iso(start_timestamp +
                                                NUM_MEASUREMENTS + 1000 * 4))
        queries = []
        ...
        for i in xrange(2):
            for timer in xrange(constants.MAX_RETRIES):
                ...
                if resp.status == 200 and len(response_body['elements']) == 1:
                    len_meas = len(
                        response_body['elements'][0]['measurements'])
                    if i == 0 and len_meas == NUM_MEASUREMENTS + 2:
                        break
                    elif i == 1 and len_meas == 4:
                        break
                    else:
    ...
    def test_list_measurements_with_endtime(self):
        time_iso = helpers.timestamp_to_iso(self._start_timestamp + 900)
        query_parms = '?name=' + str(self._names_list[0]) + \
                      '&merge_metrics=true' \
                      '&start_time=' + str(self._start_time) + \
