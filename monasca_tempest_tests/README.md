...
1. In the Tempest root dir:
    ...
    ````
	```--serial``` option is necessary here. Monasca tempest tests is not available for running in parallel (default option in ostestr) because some tests are depending on the same data.
