...
class Alarms(alarms_api_v2.AlarmsV2API, Alarming):
    ...
    def do_put_alarms(self, req, res, id):
        ...
        state, lifecycle_state, link = self._get_alarm_update_info(req, state_required=True)

        self._alarm_update(tenant_id, id, state, lifecycle_state, link)
    ...
    def do_patch_alarms(self, req, res, id):
        ...
        state, lifecycle_state, link = self._get_alarm_update_info(req)

        self._alarm_patch(tenant_id, id, state, lifecycle_state, link)
    ...
    def _alarm_update(self, tenant_id, id, new_state, lifecycle_state, link):
        ...
        old_state = self._alarms_repo.update_alarm(tenant_id, id, new_state, lifecycle_state, link)
    ...
    def _get_alarm_update_info(self, req, state_required=False):
        ...
        if 'state' in json_msg:
            ...
            if state_required:
                raise falcon.HTTPBadRequest('Bad request', 'State is required')
        if 'lifecycle_state' in json_msg:
            lifecycle_state = json_msg['lifecycle_state']
        if 'link' in json_msg:
            link = json_msg['link']

        return state, lifecycle_state, link