...
class StreamDefinitions(stream_definitions_api_v2.StreamDefinitionsV2API):
    ...
    @resource_api.Restify('/v2.0/events/stream-definitions', method='post')
    def do_post_stream_definitions(self, req, res):
        ...
        description = get_query_stream_definition_description(
            stream_definition)
        select_by = stream_definition['select_by']
        group_by = stream_definition['group_by']
        fire_criteria = stream_definition['fire_criteria']
        fire_pipeline = get_query_stream_definition_fire_pipeline(
            stream_definition)
        expire_pipeline = get_query_stream_definition_expire_pipeline(
            stream_definition)
        ...
        result = self._stream_definition_create(tenant_id, name, description,
                                                ...
                                                fire_pipeline, expire_pipeline)

        helpers.add_links_to_resource(result, req.uri)
        res.body = helpers.dumpit_utf8(result)
        res.status = falcon.HTTP_201

    @resource_api.Restify('/v2.0/events/stream-definitions/{id}', method='get')
    ...
    @resource_api.Restify('/v2.0/events/stream-definitions', method='get')
    def do_get_stream_definitions(self, req, res):
        ...
        result = self._stream_definition_list(tenant_id, name, req.uri, offset)

        res.body = helpers.dumpit_utf8(result)
        res.status = falcon.HTTP_200

    @resource_api.Restify(
        '/v2.0/events/stream-definitions/{id}', method='delete')
    ...
    def _stream_definition_create(self, tenant_id, name,
                                  description, select_by, group_by,
                                  fire_criteria, expiration,
                                  fire_actions, expire_actions,
                                  fire_pipeline, expire_pipeline):
        ...
        result = (
            {u'name': name,
             ...
             u'select_by': select_by,
             u'group_by': group_by,
             u'fire_criteria': fire_criteria,
             u'expiration': expiration,
             u'fire_pipeline': fire_pipeline,
             u'expire_pipeline': expire_pipeline,
    ...
    def _build_stream_definition_show_result(self, stream_definition_row):
        ...
        result = (
            {u'name': stream_definition_row['name'],
             ...
             u'select_by': json.loads(stream_definition_row['select_by']),
             ...
             u'fire_pipeline': stream_definition_row['fire_pipeline'],
             u'expire_pipeline': stream_definition_row['expire_pipeline'],
...
def get_query_stream_definition_fire_pipeline(stream_definition,
                                              return_none=False):
    if 'fire_pipeline' in stream_definition:
        return stream_definition['fire_pipeline']
    else:
        if return_none:
            return None
        else:
...
def get_query_stream_definition_expire_pipeline(stream_definition,
                                                return_none=False):
    if 'expire_pipeline' in stream_definition:
        return stream_definition['expire_pipeline']
    else:
        if return_none:
            return None
        else:
