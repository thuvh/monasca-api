...
class Metrics(monasca_api_v2.V2API):
    def __init__(self, global_conf):
        ...
        self._message_queue = publisher_factory.create_publisher()
        ...
        self._metric_transform = metrics_transform_factory.create_metrics_transform()
    ...
    def _transform_send_metric(self, metric, tenant_id, region):
        try:
            transformed_json_msg = self._metric_transform(metric, tenant_id, region)
            str_msg = json.dumps(transformed_json_msg, default=utils.date_handler)
    ...
    def do_post_metrics(self, req, res):
        ...
        if isinstance(metrics, list):
            for metric in metrics:
                self._transform_send_metric(metric, tenant_id, self._region)
        else:
            self._transform_send_metric(metrics, tenant_id, self._region)
