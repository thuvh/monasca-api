...
class Notifications(monasca_notifications_api_v2.NotificationsV2API):
    ...
    def __init__(self, global_conf):
        ...
        self._init_notifications_repo()

    def _init_notifications_repo(self):
        mgr = driver.DriverManager(
            namespace='monasca.repositories',
            name=cfg.CONF.repositories.notifications_driver,
            invoke_on_load=True,
            invoke_args=()
        )
        self._notifications_repo = mgr.driver

    def _read_notification(self, req):
        '''
        Read the notification from the http request and return as JSON.
    ...
    def _update_notification(self, id, tenant_id, notification):
        ...
        try:
            ...
            msg = 'No notification method exists for tenant_id = %s id = %s' % (
                tenant_id, id)
    ...
    def _list_notifications(self, tenant_id, uri):
        '''
        Lists all notification for this tenant id.
    ...
    def _list_notifications_by_id(self, tenant_id, notification_id, uri):
        '''
        ...
        '''
        try:
            notifications = self._notifications_repo.list_notifications_by_id(
                tenant_id,
                notification_id)
            return json.dumps(
    ...
    def do_post_notification_methods(self, req, res):
        ...
        notification = self._read_notification(req)
    ...
    def do_get_notification_method_by_id(self, req, res, id):
        helpers.validate_authorization(req, self._default_authorized_roles)
        tenant_id = helpers.get_tenant_id(req)
        res.body = self._list_notifications_by_id(tenant_id, id, req.uri)
        res.status = falcon.HTTP_200
