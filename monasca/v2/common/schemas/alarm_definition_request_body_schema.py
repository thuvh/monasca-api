...
alarm_definition_schema = {
    Required('name'): All(Any(str, unicode), Length(max=100)),
    Required('expression'): All(Any(str, unicode), Length(max=2048)),
    Optional('description'): All(Any(str, unicode), Length(max=250)),
    Optional('severity'): All(
        Any('low', 'med', 'high', 'critical', 'LOW', "MED", 'HIGH',
            'CRITICAL')),
    Optional('match_by'): All(Any([unicode], [str]), Length(max=255)),
    Optional('ok_actions'): All(Any([str], [unicode]), Length(max=50)),
    Optional('alarm_actions'): All(Any([str], [unicode]), Length(max=50)),
