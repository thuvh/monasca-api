        ...
        try:
            ...
            if not merge_metrics_flag:
                ...
                if len(metrics_list) > 1:
                    raise Exception(
                        "Found multiple metrics matching metric name and "
                        "dimensions. "
                        + "Please refine your search criteria using a unique "
                          "metric name or additional dimensions. "
                        + "Alternatively, you may specify "
                          "'merge_metrics=True' as a query param to combine "
                        + "all metrics matching search criteria into a "
                          "single series.")
            ...
            if 'values' in result.raw['results'][0]['series'][0]:
                ...
                measurement = {u'name': (result.raw['results'][0]
                                         ['series'][0]['name']),
                               u'id': points[-1][0],
                               u'dimensions': dimensions,
                               u'columns': [u'timestamp', u'value',
                                            u'value_meta'],
                               u'measurements': points}
        ...
        try:
            ...
            if 'values' in result.raw['results'][0]['series'][0]:
                ...
                columns = [column.replace('mean', 'avg') for column in
                           result.raw['results'][0]['series'][0]['columns']]

                name = result.raw['results'][0]['series'][0]['name']
