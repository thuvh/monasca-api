...
class MetricsRepository(metrics_repository.MetricsRepository):
    ...
    def _build_from_clause(self, dimensions, name, tenant_id, region,
                           start_timestamp=None, end_timestamp=None):
        from_clause = 'from '
        from_clause += self._build_regex_clause(dimensions, name, tenant_id, region,
    ...
    def measurement_list(self, tenant_id, region, name, dimensions,
        ...
        try:
            ...
            try:
                result = self.influxdb_client.query(query, 'm')
            ...
            for serie in result:
                ...
                fmtd_pts = [['%s.%03d' % (time.strftime('%Y-%m-%dT%H:%M:%SZ',
                    time.gmtime(point[0]/1000)), point[0]%1000), point[1],
