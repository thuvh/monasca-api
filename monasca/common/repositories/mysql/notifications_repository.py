...
class NotificationsRepository(
    ...
    def list_notifications(self, tenant_id):
        try:
            ...
            notifications = []
            for result in results:
                notifications.append(self.notification_from_result(result))
    ...
    def list_notifications_by_id(self, tenant_id, notification_id):
        try:
            result = Notification_Method.get(
                (Notification_Method.tenant_id == tenant_id) & (
                    Notification_Method.id == notification_id))
            return (self.notification_from_result(result))
        except Notification_Method.DoesNotExist as dnex:
            raise exceptions.DoesNotExistException(str(dnex))
        except Exception as ex:
            LOG.exception(str(ex))
            raise exceptions.RepositoryException(str(ex))
    ...
    def update_notification(
        ...
        try:
            ...
            if num_rows_updated == 0:
                raise exceptions.DoesNotExistException('404 id not found')
