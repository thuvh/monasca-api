...
### Group Rule
Alarm Grouping is when a notification is grouped and sent to a notification method defined by the grouping rule. A group rule does
not effect the notification actions defined in the alarm definition. Therefor, an alarm that has alarm actions specified in both the
alarm definition and the group rule will trigger two notifications to be sent if it transitions to the ALARM state. An alarm is matched
to a group rule by expression. Please see [Group Rule Matching](# Group Rule Matching) for more information. A group rule also has
...
This will result in two group notifications being sent. The "repeat" parameter specifies the amount of time the notification should wait
before it is re-sent by the Periodic Engine. Notification actions can also be specified for the alarm transitioning to any state by
using the parameters: "alarm\_actions", "ok\_actions", and "undetermined\_actions".

### Inhibit Rule
Inhibit is when one alarm in the ALARM state causes another class of alarms to not send notifications. When an inhibit rule is
...
for keeping those tags up-to-date. A source alarm inhibits a target alarm. If the source alarm is in ALARM state, then the target alarm does
not send any notifications. If the source alarm is in  OK or UNDETERMINED state, then the target alarm will send notifications like normal.
In the case where a source and target alarm both transition back to OK or UNDETERMINED, the target alarm will not send a notification.
There is a window where, for a period of time after a source alarm transitions out of ALARM, no target alarm notification will be sent.
Any alarm matching an inhibition rule must wait for some configured wait time to ensure proper behavior. See the Notification Manager
...
Engine is responsible for keeping the database up-to-date. A silence rule has the parameters "start\_time" and "silence\_duration". Together
these parameters define when the rule is active and valid. The start time should be in ISO 8601 combined date and time format in UTC.
The silence duration is a string built of non delimited value and labels from the table below. For example "1y2d3h4m5s" would define the duration
of one year and two days and three hours and four minutes and five seconds. Each label is preceded by an integer amount of that label, and
there should be no spaces between each value and label pair in the silence duration string.
...
```
group by hostname excluding cpu_perc
```
...
```
group by __alarmName__, hostname excluding {__alarmName__=cpu_perc_high, __metricName__=cpu_perc}
```
...
If two rules are applied to the same alarm, is the alarm sent?
...
```
{"alarm-silencing-definition-created":{
     "name": "silence_rule_1",
     "matchers": {
         "\_\_severity\_\_": "LOW"},
     "start_time": "2017-02-21 20:00:00",
     "end_time": "2017-02-21 22:00:00"}}
```

Output: AT2 gets silenced and AT1 sends a notification. NOTE: SilenceRule1 expires after "end\_time"="2017-02-21 22:00:00".
#####2.    Inhibit Example
Three alarm transitions: AT1, AT2 and AT3

|| AT1 | AT2 | AT3
|| --- | --- | ---
**\_\_alarmId\_\_** | "d42bc" | "d42bc" | "d42bc"
**\_\_severity\_\_** | HIGH | LOW | LOW
**\_\_alarmName\_\_** | "cpu high" | "memory high" | "vm is dead"
**\_\_state\_\_** | ALARM | ALARM | ALARM

InhibitionRule1 =

```
{"inhibit-rule-created": {
     "name": "inhibit_rule_1",
     "source":{
         "\_\_severity\_\_":"HIGH"},
     "target":{
         "\_\_severity\_\_":"LOW"},
     "equal":["\_\_alarmId\_\_"],
     "exclusions": {
         "\_\_alarmName\_\_": "vm is dead"}}}
```
...
GroupingRule1:
```
{"alarm-grouping-definition-created": {
    "name": "group_rule_2",
    "matchers": ["hostname"],
    "id": "b7163",
    "repeat_interval": "2h",
...
{"alarm-silencing-definition-created": {
    ...
    "matchers": {
        "\_\_severity\_\_": "LOW"},
    "start_time": "1487269470498",
    "end_time": "1587269470498"}}
```
GroupingRule2:
```
{"alarm-grouping-definition-created": {
...
**\_\_alarmDefinitionId\_\_** | "d42bc" | "d42bc"
**\_\_severity\_\_** | HIGH | LOW
...
{"inhibit-rule-created": {
    ...
    "target":{
        "\_\_severity\_\_":"LOW"},
    "equal":["\_\_alarmDefinitionId\_\_"]}}
```
...
|| AT1 | AT2 | AT3
|| --- | --- | ---
**\_\_alarmDefinitionId\_\_** | "d42bc" | "d42bc" | "d42bc"
**\_\_severity\_\_** | HIGH | LOW | HIGH
...
{"alarm-grouping-definition-created": {
    ...
    "undetermined_actions": ["cf892"]}}
```
Output: AT2 gets inhibited because its severity is low and will not be included in the group. AT3 is the source alarm. Since AT1 is in OK state,
it is not a source alarm. For grouping, AT1 and AT3 has the same host name but different state. So there will be two grouped notifications sent
out: “group\_notification\_rule\_1\_host1\_ok[1]” and “group\_notification\_rule\_1\_host1\_alarm[1]”.
#####7.    Silence, Inhibit and Group Example
Four alarm transitions: AT1, AT2, AT3 and AT5

|| AT1 | AT2 | AT3 | AT5
|| --- | --- | --- | ---
**\_\_alarmDefinitionId\_\_** | "d42bc" | "d42bc" | "d42bc" | "d42bc"
**\_\_state\_\_** | ALARM | ALARM | OK | UNDETERMINED
**\_\_severity\_\_** | HIGH | LOW | HIGH | HIGH
**hostname** | host1 | host2 | host1 | host3
**\_\_alarm_actions\_\_** | [] | [] | [] | []
**\_\_ok_actions\_\_** | [] | [] | [] | []
...
{"inhibit-rule-created": {
    ...
    "equal":["\_\_alarmDefinitionId\_\_"]}}
```
Output: AT1 is in "group\_notification\_rule\_1\_host1\_alarm" group and silenced. AT3 is in "group\_notification\_rule\_1\_host1\_ok" group
and silenced. AT2 is in "group\_notification\_rule\_1\_host2\_alarm" group and inhibited. AT5 is in
"group\_notification\_rule\_1\_host3\_undetermined" group and will send notification “group\_notification\_rule\_1\_host3\_undetermined[1]”
...
* undetermined\_actions ([string(50)], optional) - Array of notification method IDs that are invoked when the group of alarms transition to the `UNDETERMINED` state.
...

{
	"name": "group_rule_1",
        ...
        "expression": "group by __alarmName__, __metricName__, hostName excluding {__alarmName__=cpu_too_high, hostname=host1}",
	"group_wait": "10s",
	"repeat_interval": "1h",
...
#### Request Examples
```
GET /v2.0/group-rules?name=group_rule HTTP/1.1
Host: 192.168.10.4:8070
Content-Type: application/json
X-Auth-Token: 2b8882ba2ec44295bf300aecb2caa4f7
Cache-Control: no-cache
...
```
{
  ...
  "elements": [
    {
      "expression": "group by __alarmName__, __metricName__, hostName excluding {hostname=host2}",
      "alarm_actions": [
        "bf718275-a483-43eb-b125-eb31058d3053"
      ],
      "ok_actions": [
    ...
    {
      "expression": "group by hostName excluding {hostname=host4}",
      "alarm_actions": [
        "bf718275-a483-43eb-b125-eb31058d3053"
      ],
      "ok_actions": [
...
#### Request Examples
```
GET /v2.0/silence-rules?name=silence_rule HTTP/1.1
Host: 192.168.10.4:8070
Content-Type: application/json
X-Auth-Token: 2b8882ba2ec44295bf300aecb2caa4f7
Cache-Control: no-cache
...
* expression (string(1024)) - The rule expression. Note, expression must not change.
