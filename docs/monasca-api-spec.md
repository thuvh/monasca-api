...
### Text Representation
In this document, metrics will be represented in the form `name{name=value}` where name is the metric name and the name=value pairs in the curly braces are the dimensions. For example, `cpu.idle_perc{service=monitoring}` represents a metric with the name "cpu.idle_perc" and the dimension "service=monitoring".
...
```
avg(cpu.idle_perc{service=monitoring,hostname=mini-mon})
avg(cpu.idle_perc{service=monitoring,hostname=devstack})
avg(cpu.user_perc{service=monitoring,hostname=mini-mon}) 
avg(cpu.user_perc{service=monitoring,hostname=devstack}) 
```
