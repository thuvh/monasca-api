...
# Health Check
The Monasca ApI comes with a built-in health check mechanism. It is available in two flavours, both accessible
...
### Peripheral checks
* __Kafka__ is consider healthy if connection to broker can be established and configured topics can be found.
* __Alarm database__ (MariaDB/MySQL, PostgreSQL) is consider healthy if connection to database can be established
  and sample query can be executed.
* __Metrics database__ (InfluxDB, Cassandra) is consider healthy if: `InfluxDB` is set health check is verified according to the
InfluxDB documentation (http://192.168.10.6/ping), `Cassandra` is set health check is verified through new connection to the database.
