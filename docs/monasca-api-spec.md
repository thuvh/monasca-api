...
not effect the notification actions defined in the alarm definition. Therefore, an alarm that has alarm actions specified in both the
...
This will result in two group notifications being sent. The "repeat_interval" parameter specifies the amount of time the notification should wait
...
Any alarm matching an inhibit rule must wait for some configured wait time to ensure proper behavior. See the Notification Manager
...
The silence duration is a string built of non delimited value and labels from the table below. For example "2d3h4m5s" would define the duration
of two days and three hours and four minutes and five seconds. Each label is preceded by an integer amount of that label, and
