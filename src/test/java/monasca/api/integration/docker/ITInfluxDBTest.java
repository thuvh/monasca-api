...
public class ITInfluxDBTest {
  ...
  private void runAPI() throws Exception {
    ...
    ProcessBuilder pb = new ProcessBuilder("java", "-cp", "./target/" + latestShadedJarFileName,
        "monasca.api.MonApiApplication", "server", "src/test/resources/monasca-api-config.yml");
    File log = new File("monasca-api-integration-test.log");
  ...
  private String getLatestShadedJarFileName() {
    ...
    FileFilter fileFilter = new RegexFileFilter("^monasca-api-0\\.1\\.0-(\\d|\\w)+-(\\d|\\w)+\\.jar");
    File[] files = dir.listFiles(fileFilter);
    if (files.length == 0) {
      System.err.println("Failed to find shaded jar. You must build monasca-api before running this "
